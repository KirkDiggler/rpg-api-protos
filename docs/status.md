...
- **Intimidate, the first shenanigan (rpg-project#454, 2026-09-16)** — additive
  ...
  paused, roll, saved, delivery }`, `VERB_INTIMIDATE = 8`, and `EVENT_KIND_INTIMIDATED = 32`
  ...
  for a held offer, the same public roll — plus the two S6 reports Unlock
  omits and every other mutating verb here keeps. It carries **nothing about what the
...
- **Explicit Death Saves on the session wire (rpg-api-protos#277,
  ...
  is explicit independently of optional progress; the authenticated-owner
  v1alpha2 `CharacterData` reuses the same types at tags 15/16. Remaining counts
  are provider-derived and no client threshold is exposed. Response and event
  share one authoritative roll and opaque generated `presentation_id`; the
  token contains no Story/global sequence, presentation retries retain the same
  result, and presentation `authority_seq` remains separate from every
  recipient-local event/response sequence. Proto/docs only; API and web
  adoption remain follow-on work.
