...
// How the hit points gained at a level are determined.
//
// The level-1-only "maximum hit die" method is deliberately absent: it is a
// creation rule, and this enum is only ever read by a level-up.
type HitPointMethod int32

const (
	HitPointMethod_HIT_POINT_METHOD_UNSPECIFIED HitPointMethod = 0
	// Roll the class hit die
	HitPointMethod_HIT_POINT_METHOD_ROLLED HitPointMethod = 1
	// Take the hit die's fixed average
	HitPointMethod_HIT_POINT_METHOD_AVERAGE HitPointMethod = 2
)

// Enum value maps for HitPointMethod.
var (
	HitPointMethod_name = map[int32]string{
		0: "HIT_POINT_METHOD_UNSPECIFIED",
		1: "HIT_POINT_METHOD_ROLLED",
		2: "HIT_POINT_METHOD_AVERAGE",
	}
	HitPointMethod_value = map[string]int32{
		"HIT_POINT_METHOD_UNSPECIFIED": 0,
		"HIT_POINT_METHOD_ROLLED":      1,
		"HIT_POINT_METHOD_AVERAGE":     2,
	}
)

func (x HitPointMethod) Enum() *HitPointMethod {
	p := new(HitPointMethod)
	*p = x
	return p
}

func (x HitPointMethod) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (HitPointMethod) Descriptor() protoreflect.EnumDescriptor {
	return file_dnd5e_api_v1alpha1_character_proto_enumTypes[2].Descriptor()
}

func (HitPointMethod) Type() protoreflect.EnumType {
	return &file_dnd5e_api_v1alpha1_character_proto_enumTypes[2]
}

func (x HitPointMethod) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use HitPointMethod.Descriptor instead.
func (HitPointMethod) EnumDescriptor() ([]byte, []int) {
...
// Deprecated: fixed enum taxonomy. Use dnd5e.api.v1alpha2.encounter.SlotDef (server-owned
// data, open vocabulary) instead. See rpg-api-protos#187.
...
type CharacterFeature struct {
	...
	Id          FeatureId  `protobuf:"varint,1,opt,name=id,proto3,enum=dnd5e.api.v1alpha1.FeatureId" json:"id,omitempty"`
	Name        string     `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	Description string     `protobuf:"bytes,3,opt,name=description,proto3" json:"description,omitempty"`
	Source      string     `protobuf:"bytes,4,opt,name=source,proto3" json:"source,omitempty"`                                                               // Full ref format: "dnd5e:classes:barbarian"
	Level       int32      `protobuf:"varint,5,opt,name=level,proto3" json:"level,omitempty"`                                                                // Level when gained (0 for racial/background)
	ActionType  ActionType `protobuf:"varint,6,opt,name=action_type,json=actionType,proto3,enum=dnd5e.api.v1alpha1.ActionType" json:"action_type,omitempty"` // Action economy cost to activate
	// Raw JSON from toolkit for UI display (charges remaining, damage bonuses, etc.)
	FeatureData   []byte `protobuf:"bytes,7,opt,name=feature_data,json=featureData,proto3" json:"feature_data,omitempty"`
...
func (x *CharacterFeature) GetId() FeatureId {
	if x != nil {
		return x.Id
	}
...
func (x *CharacterFeature) GetActionType() ActionType {
	if x != nil {
		return x.ActionType
	}
...
func (x *CharacterFeature) GetFeatureData() []byte {
	if x != nil {
...
type Character struct {
	...
	// The toolkit's cumulative experience total, written by the API from the
	// stored sheet. Cumulative and never debited: the total only grows.
	//
	// Read-only over the wire. No RPC on any service writes it. Experience is
	// granted inside the toolkit — today by the fall of a monster, divided among
	// the party by the session SDK — and a client watches a grant land on the
	// session stream's EVENT_KIND_EXPERIENCE_GAINED rather than polling this
	// field.
	...
	// Deprecated: use canonical recoverable resources. Retained for wire
	// compatibility only.
	//
	...
	ActiveConditions []*Condition `protobuf:"bytes,28,rep,name=active_conditions,json=activeConditions,proto3" json:"active_conditions,omitempty"`
	// Visual appearance customization
	Appearance *Appearance `protobuf:"bytes,29,opt,name=appearance,proto3" json:"appearance,omitempty"`
	// The cantrips this character knows, as canonical spell refs —
	// "dnd5e:spells:true-strike". The sheet has held them since creation
	// (character.Data.KnownCantrips); this is the first field that says so on
	// the wire. Refs, never the Spell enum, for the reason SpellOptions gives.
	//
	// KNOWN IS NOT CASTABLE. A ref here says the character learned the spell,
	// not that this build can cast it right now; what can be cast is compiled
	// per turn and read from Afford, which is the only surface that answers it.
	KnownCantrips []string `protobuf:"bytes,30,rep,name=known_cantrips,json=knownCantrips,proto3" json:"known_cantrips,omitempty"`
	// The levelled spells this character knows, as canonical spell refs. Same
	// vocabulary and same law as known_cantrips: knowing is not casting.
	KnownSpells []string `protobuf:"bytes,31,rep,name=known_spells,json=knownSpells,proto3" json:"known_spells,omitempty"`
	// The level this character's experience entitles it to, derived by the API
	// from the toolkit's threshold table (2014 PHB p.15: 0, 300, 900, 2700, ...).
	//
	// ENTITLEMENT IS NOT LEVEL. It is what the character MAY take, and the gap
	// between it and `level` is the whole "level up available" signal: no flag,
	// no stored state, nothing to keep in sync. An entitled character that keeps
	// playing without levelling is one whose gap is simply still open.
	//
	// Derived and read-only; never stored, never sent by a client. A loaded
	// character is always entitled to at least 1, so 0 means "not projected",
	// never "level 0".
	EntitledLevel int32 `protobuf:"varint,32,opt,name=entitled_level,json=entitledLevel,proto3" json:"entitled_level,omitempty"`
	// The cumulative experience total at which the next level becomes available,
	// derived by the API from the same threshold table.
	//
	// Derived and read-only. 0 means there is no next level, because the
	// character is at the top of the table — not that the next level is free.
	NextLevelThreshold int32 `protobuf:"varint,33,opt,name=next_level_threshold,json=nextLevelThreshold,proto3" json:"next_level_threshold,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}
...
func (x *Character) GetActiveConditions() []*Condition {
	if x != nil {
...
func (x *Character) GetAppearance() *Appearance {
	if x != nil {
		return x.Appearance
	}
	return nil
...
func (x *Character) GetKnownCantrips() []string {
	if x != nil {
		return x.KnownCantrips
	}
	return nil
}

func (x *Character) GetKnownSpells() []string {
	if x != nil {
		return x.KnownSpells
	}
	return nil
}

func (x *Character) GetEntitledLevel() int32 {
	if x != nil {
		return x.EntitledLevel
	}
	return 0
}

func (x *Character) GetNextLevelThreshold() int32 {
	if x != nil {
		return x.NextLevelThreshold
	}
	return 0
}
...
//
// The legacy color fields at tags 1–4 are retained for source and wire
// compatibility. They are deprecated and inert; clients must not use them.
// Active customization is expressed through the provider-neutral messages
// below. The toolkit owns semantic validation and public projection; the API
// converts and delegates.
type Appearance struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Deprecated and inert legacy color field; not part of active customization.
	//
	// Deprecated: Marked as deprecated in dnd5e/api/v1alpha1/character.proto.
	SkinTone string `protobuf:"bytes,1,opt,name=skin_tone,json=skinTone,proto3" json:"skin_tone,omitempty"`
	// Deprecated and inert legacy color field; not part of active customization.
	//
	// Deprecated: Marked as deprecated in dnd5e/api/v1alpha1/character.proto.
	...
	Hair *v1alpha1.HairCustomization `protobuf:"bytes,5,opt,name=hair,proto3" json:"hair,omitempty"`
	// Provider-neutral class-outfit color intent. Absent channels preserve the
	// provider's authored channel.
	Outfit        *v1alpha1.OutfitCustomization `protobuf:"bytes,6,opt,name=outfit,proto3" json:"outfit,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
// Deprecated: Marked as deprecated in dnd5e/api/v1alpha1/character.proto.
func (x *Appearance) GetSkinTone() string {
	if x != nil {
		return x.SkinTone
	}
	return ""
...
// Deprecated: Marked as deprecated in dnd5e/api/v1alpha1/character.proto.
func (x *Appearance) GetEyeColor() string {
	if x != nil {
		return x.EyeColor
	}
	return ""
...
func (x *Appearance) GetHair() *v1alpha1.HairCustomization {
	if x != nil {
		return x.Hair
	}
	return nil
...
func (x *Appearance) GetOutfit() *v1alpha1.OutfitCustomization {
	if x != nil {
		return x.Outfit
	}
	return nil
...
type Proficiencies struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Skills whose provider-authored rank doubles the proficiency contribution.
	// These also appear in skills. Includes Expertise and equivalent grants such
	// as Blessings of Knowledge; it does not identify a particular class feature.
	ExpertiseSkills []Skill `protobuf:"varint,9,rep,packed,name=expertise_skills,json=expertiseSkills,proto3,enum=dnd5e.api.v1alpha1.Skill" json:"expertise_skills,omitempty"`
	// Skill proficiencies
	...
	ArmorCategories []ArmorProficiencyCategory `protobuf:"varint,6,rep,packed,name=armor_categories,json=armorCategories,proto3,enum=dnd5e.api.v1alpha1.ArmorProficiencyCategory" json:"armor_categories,omitempty"`
	// Weapon proficiency categories (simple, martial)
	WeaponCategories []WeaponProficiencyCategory `protobuf:"varint,7,rep,packed,name=weapon_categories,json=weaponCategories,proto3,enum=dnd5e.api.v1alpha1.WeaponProficiencyCategory" json:"weapon_categories,omitempty"`
	// Specific weapon proficiencies (for individual weapons like longsword)
	SpecificWeapons []Weapon `protobuf:"varint,8,rep,packed,name=specific_weapons,json=specificWeapons,proto3,enum=dnd5e.api.v1alpha1.Weapon" json:"specific_weapons,omitempty"`
	// Tool proficiencies
...
func (x *Proficiencies) GetExpertiseSkills() []Skill {
	if x != nil {
		return x.ExpertiseSkills
	}
	return nil
}
...
func (x *Proficiencies) GetArmorCategories() []ArmorProficiencyCategory {
	if x != nil {
...
func (x *Proficiencies) GetWeaponCategories() []WeaponProficiencyCategory {
	if x != nil {
...
func (x *Proficiencies) GetSpecificWeapons() []Weapon {
	if x != nil {
		return x.SpecificWeapons
	}
	return nil
}
...
// Request for the level this character would take next
type GetNextLevelRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The character to describe the next level for
	CharacterId   string `protobuf:"bytes,1,opt,name=character_id,json=characterId,proto3" json:"character_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetNextLevelRequest) Reset() {
	*x = GetNextLevelRequest{}
	mi := &file_dnd5e_api_v1alpha1_character_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetNextLevelRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetNextLevelRequest) ProtoMessage() {}

func (x *GetNextLevelRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dnd5e_api_v1alpha1_character_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use GetNextLevelRequest.ProtoReflect.Descriptor instead.
func (*GetNextLevelRequest) Descriptor() ([]byte, []int) {
	return file_dnd5e_api_v1alpha1_character_proto_rawDescGZIP(), []int{18}
}

func (x *GetNextLevelRequest) GetCharacterId() string {
	if x != nil {
		return x.CharacterId
	}
	return ""
...
// What the character's next level brings.
//
// Everything a level-up screen needs to be either a form or a confirmation,
// with no class-specific branch in it. A level that requires nothing is still
// a correct screen: it has features, a hit die and pools to show, so a fighter
// reads "Level 2: Action Surge" rather than a bare button.
type GetNextLevelResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The class level the character would take, one above its current level
	Level int32 `protobuf:"varint,1,opt,name=level,proto3" json:"level,omitempty"`
	// The class the level is taken in
	Class Class `protobuf:"varint,2,opt,name=class,proto3,enum=dnd5e.api.v1alpha1.Class" json:"class,omitempty"`
	// The choices this level requires, in the same Choice message character
	// creation renders. A level that asks nothing returns none.
	Choices []*Choice `protobuf:"bytes,3,rep,name=choices,proto3" json:"choices,omitempty"`
	// The features this level grants
	Features []*FeatureInfo `protobuf:"bytes,4,rep,name=features,proto3" json:"features,omitempty"`
	// Hit die of the class the level is taken in (e.g., 10 for d10)
	HitDie        int32 `protobuf:"varint,5,opt,name=hit_die,json=hitDie,proto3" json:"hit_die,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetNextLevelResponse) Reset() {
	*x = GetNextLevelResponse{}
	mi := &file_dnd5e_api_v1alpha1_character_proto_msgTypes[19]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetNextLevelResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetNextLevelResponse) ProtoMessage() {}

func (x *GetNextLevelResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dnd5e_api_v1alpha1_character_proto_msgTypes[19]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use GetNextLevelResponse.ProtoReflect.Descriptor instead.
func (*GetNextLevelResponse) Descriptor() ([]byte, []int) {
	return file_dnd5e_api_v1alpha1_character_proto_rawDescGZIP(), []int{19}
}

func (x *GetNextLevelResponse) GetLevel() int32 {
	if x != nil {
		return x.Level
	}
	return 0
...
func (x *GetNextLevelResponse) GetClass() Class {
	if x != nil {
		return x.Class
	}
	return Class_CLASS_UNSPECIFIED
...
func (x *GetNextLevelResponse) GetChoices() []*Choice {
	if x != nil {
		return x.Choices
	}
	return nil
...
func (x *GetNextLevelResponse) GetFeatures() []*FeatureInfo {
	if x != nil {
		return x.Features
	}
	return nil
...
func (x *GetNextLevelResponse) GetHitDie() int32 {
	if x != nil {
		return x.HitDie
	}
	return 0
...
// Request to take the character's next level.
//
// One atomic call and no draft: the screen holds its own state until it sends
// this. The engine validates the selections the way creation validates a draft
// — count, options, category — and refuses a level the character's experience
// does not entitle it to.
type LevelUpRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The character taking the level
	CharacterId string `protobuf:"bytes,1,opt,name=character_id,json=characterId,proto3" json:"character_id,omitempty"`
	// How the hit points for this level are determined
	HitPointMethod HitPointMethod `protobuf:"varint,2,opt,name=hit_point_method,json=hitPointMethod,proto3,enum=dnd5e.api.v1alpha1.HitPointMethod" json:"hit_point_method,omitempty"`
	// The selections for the choices GetNextLevel asked for, and nothing else.
	//
	// Level-up choices carry CHOICE_SOURCE_CLASS. The level record already says
	// which level a choice belongs to, so the source does not have to.
	Choices       []*ChoiceData `protobuf:"bytes,3,rep,name=choices,proto3" json:"choices,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *LevelUpRequest) GetCharacterId() string {
	if x != nil {
		return x.CharacterId
	}
	return ""
...
func (x *LevelUpRequest) GetHitPointMethod() HitPointMethod {
	if x != nil {
		return x.HitPointMethod
	}
	return HitPointMethod_HIT_POINT_METHOD_UNSPECIFIED
}

func (x *LevelUpRequest) GetChoices() []*ChoiceData {
	if x != nil {
		return x.Choices
	}
	return nil
...
// Response from taking a level
type LevelUpResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The character after the level was taken
	Character *Character `protobuf:"bytes,1,opt,name=character,proto3" json:"character,omitempty"`
	// What the level brought
	Gained        *LevelGained `protobuf:"bytes,2,opt,name=gained,proto3" json:"gained,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *LevelUpResponse) GetCharacter() *Character {
	if x != nil {
		return x.Character
	}
	return nil
...
func (x *LevelUpResponse) GetGained() *LevelGained {
	if x != nil {
		return x.Gained
	}
	return nil
...
// What a level brought, as the engine reported it
type LevelGained struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The class level that was taken
	Level int32 `protobuf:"varint,1,opt,name=level,proto3" json:"level,omitempty"`
	// Hit points this level added to the maximum
	HitPointsGained int32 `protobuf:"varint,2,opt,name=hit_points_gained,json=hitPointsGained,proto3" json:"hit_points_gained,omitempty"`
	// The features this level granted
	Features []*FeatureInfo `protobuf:"bytes,3,rep,name=features,proto3" json:"features,omitempty"`
	// Every pool whose maximum this level changed. A pool that did not change is
	// absent, so an empty list means the level changed no pool.
	ResourceChanges []*ResourceMaximumChange `protobuf:"bytes,4,rep,name=resource_changes,json=resourceChanges,proto3" json:"resource_changes,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *LevelGained) Reset() {
	*x = LevelGained{}
...
func (x *LevelGained) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LevelGained) ProtoMessage() {}

func (x *LevelGained) ProtoReflect() protoreflect.Message {
	mi := &file_dnd5e_api_v1alpha1_character_proto_msgTypes[22]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use LevelGained.ProtoReflect.Descriptor instead.
func (*LevelGained) Descriptor() ([]byte, []int) {
	return file_dnd5e_api_v1alpha1_character_proto_rawDescGZIP(), []int{22}
}

func (x *LevelGained) GetLevel() int32 {
	if x != nil {
		return x.Level
	}
	return 0
...
func (x *LevelGained) GetHitPointsGained() int32 {
	if x != nil {
		return x.HitPointsGained
	}
	return 0
}

func (x *LevelGained) GetFeatures() []*FeatureInfo {
	if x != nil {
		return x.Features
	}
	return nil
...
func (x *LevelGained) GetResourceChanges() []*ResourceMaximumChange {
	if x != nil {
...
// A pool whose maximum a level changed.
//
// Reported by the level-up rather than read back off the sheet, because spell
// slots have no persistent representation on any wire: v1alpha1
// Character.spell_slots is deprecated and unread, and
// dnd5e.api.v1alpha2.encounter.ResourceView excludes slots by design. This is
// what lets a confirmation say "1st-level spell slots 2 to 3".
type ResourceMaximumChange struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Opaque rulebook core/resources.ResourceKey, the same vocabulary
	// dnd5e.api.v1alpha2.encounter.ResourceView.key carries
	Key string `protobuf:"bytes,1,opt,name=key,proto3" json:"key,omitempty"`
	// Display name for the pool
	Name string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	// The maximum before the level was taken
	PreviousMaximum int32 `protobuf:"varint,3,opt,name=previous_maximum,json=previousMaximum,proto3" json:"previous_maximum,omitempty"`
	// The maximum after the level was taken
	NewMaximum    int32 `protobuf:"varint,4,opt,name=new_maximum,json=newMaximum,proto3" json:"new_maximum,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ResourceMaximumChange) Reset() {
	*x = ResourceMaximumChange{}
	mi := &file_dnd5e_api_v1alpha1_character_proto_msgTypes[23]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ResourceMaximumChange) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ResourceMaximumChange) ProtoMessage() {}

func (x *ResourceMaximumChange) ProtoReflect() protoreflect.Message {
	mi := &file_dnd5e_api_v1alpha1_character_proto_msgTypes[23]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use ResourceMaximumChange.ProtoReflect.Descriptor instead.
func (*ResourceMaximumChange) Descriptor() ([]byte, []int) {
	return file_dnd5e_api_v1alpha1_character_proto_rawDescGZIP(), []int{23}
}

func (x *ResourceMaximumChange) GetKey() string {
	if x != nil {
		return x.Key
	}
	return ""
...
func (x *ResourceMaximumChange) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
...
func (x *ResourceMaximumChange) GetPreviousMaximum() int32 {
	if x != nil {
		return x.PreviousMaximum
	}
	return 0
}

func (x *ResourceMaximumChange) GetNewMaximum() int32 {
	if x != nil {
		return x.NewMaximum
	}
	return 0
...
type CharacterDraftData struct {
	...
	// Visual appearance customization
	Appearance    *Appearance `protobuf:"bytes,13,opt,name=appearance,proto3" json:"appearance,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *CharacterDraftData) GetAppearance() *Appearance {
	if x != nil {
		return x.Appearance
	}
	return nil
...
type CharacterDraft struct {
	...
	// Visual appearance customization
	Appearance    *Appearance `protobuf:"bytes,18,opt,name=appearance,proto3" json:"appearance,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *CharacterDraft) GetAppearance() *Appearance {
	if x != nil {
		return x.Appearance
	}
	return nil
...
type UpdateAppearanceRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	DraftId       string                 `protobuf:"bytes,1,opt,name=draft_id,json=draftId,proto3" json:"draft_id,omitempty"`
	Appearance    *Appearance            `protobuf:"bytes,2,opt,name=appearance,proto3" json:"appearance,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *UpdateAppearanceRequest) GetDraftId() string {
	if x != nil {
		return x.DraftId
	}
	return ""
...
func (x *UpdateAppearanceResponse) GetDraft() *CharacterDraft {
	if x != nil {
		return x.Draft
	}
	return nil
...
type ClassInfo struct {
	...
	SpecificWeaponProficiencies []Weapon  `protobuf:"varint,8,rep,packed,name=specific_weapon_proficiencies,json=specificWeaponProficiencies,proto3,enum=dnd5e.api.v1alpha1.Weapon" json:"specific_weapon_proficiencies,omitempty"`
	ToolProficiencies           []Tool    `protobuf:"varint,9,rep,packed,name=tool_proficiencies,json=toolProficiencies,proto3,enum=dnd5e.api.v1alpha1.Tool" json:"tool_proficiencies,omitempty"`
	SavingThrowProficiencies    []Ability `protobuf:"varint,10,rep,packed,name=saving_throw_proficiencies,json=savingThrowProficiencies,proto3,enum=dnd5e.api.v1alpha1.Ability" json:"saving_throw_proficiencies,omitempty"`
	// Class features at level 1
	Level_1Features []*FeatureInfo `protobuf:"bytes,11,rep,name=level_1_features,json=level1Features,proto3" json:"level_1_features,omitempty"`
	// Spellcasting information (if applicable)
	Spellcasting *SpellcastingInfo `protobuf:"bytes,12,opt,name=spellcasting,proto3" json:"spellcasting,omitempty"`
	// ALL requirements (choices) - skills, equipment, etc.
	// This is the ONE place for all choices
	Choices []*Choice `protobuf:"bytes,13,rep,name=choices,proto3" json:"choices,omitempty"`
	// Subclass options for this class (only populated for base classes like Cleric, not for subclasses)
	Subclasses    []*SubclassInfo `protobuf:"bytes,14,rep,name=subclasses,proto3" json:"subclasses,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
type SpellInfo struct {
	...
	SpellId Spell `protobuf:"varint,1,opt,name=spell_id,json=spellId,proto3,enum=dnd5e.api.v1alpha1.Spell" json:"spell_id,omitempty"`
	// Display information
	Name        string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	Description string `protobuf:"bytes,3,opt,name=description,proto3" json:"description,omitempty"`
	// Spell level (0 for cantrips, 1-9 for leveled spells)
	Level int32 `protobuf:"varint,4,opt,name=level,proto3" json:"level,omitempty"`
	// Canonical shared spell identity; use this to join class choices and sheet refs.
	SpellRef string `protobuf:"bytes,5,opt,name=spell_ref,json=spellRef,proto3" json:"spell_ref,omitempty"`
	// This catalog spell may be selected or granted, but has no executable
	// behavior yet. Display "Not yet implemented"; this does not grant a cast.
	NotYetImplemented bool `protobuf:"varint,6,opt,name=not_yet_implemented,json=notYetImplemented,proto3" json:"not_yet_implemented,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *SpellInfo) Reset() {
	*x = SpellInfo{}
...
func (x *SpellInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SpellInfo) ProtoMessage() {}

func (x *SpellInfo) ProtoReflect() protoreflect.Message {
	mi := &file_dnd5e_api_v1alpha1_character_proto_msgTypes[64]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use SpellInfo.ProtoReflect.Descriptor instead.
func (*SpellInfo) Descriptor() ([]byte, []int) {
	return file_dnd5e_api_v1alpha1_character_proto_rawDescGZIP(), []int{64}
}

func (x *SpellInfo) GetSpellId() Spell {
	if x != nil {
		return x.SpellId
	}
...
func (x *SpellInfo) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
...
func (x *SpellInfo) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
...
func (x *SpellInfo) GetLevel() int32 {
	if x != nil {
		return x.Level
	}
	return 0
...
func (x *SpellInfo) GetSpellRef() string {
	if x != nil {
		return x.SpellRef
	}
	return ""
...
func (x *SpellInfo) GetNotYetImplemented() bool {
	if x != nil {
		return x.NotYetImplemented
	}
	return false
}
...
type BackgroundInfo struct {
	...
	// ALL requirements (choices) - equipment, tool proficiencies, etc.
	// This is the ONE place for all choices, same pattern as
	// ClassInfo.choices (field 13 above) — mirrors the toolkit's
	// choices.GetBackgroundRequirements, which now exists alongside
	// GetRaceRequirements/GetClassRequirements (rpg-toolkit#1554).
	// Eight backgrounds have a real choice here (equipment and/or tool
	// proficiency, varying per background) as of rulebooks/dnd5e/v0.146.0;
	// every other background leaves this empty.
	Choices       []*Choice `protobuf:"bytes,16,rep,name=choices,proto3" json:"choices,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *BackgroundInfo) GetChoices() []*Choice {
	if x != nil {
		return x.Choices
	}
	return nil
...
type GetRequirementsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Requirements  []*Choice              `protobuf:"bytes,1,rep,name=requirements,proto3" json:"requirements,omitempty"` // From choices.proto
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *GetRequirementsResponse) GetRequirements() []*Choice {
	if x != nil {
...
type SubmitChoicesResponse struct {
	...
	RemainingRequirements []*Choice              `protobuf:"bytes,3,rep,name=remaining_requirements,json=remainingRequirements,proto3" json:"remaining_requirements,omitempty"` // From choices.proto
	unknownFields         protoimpl.UnknownFields
	sizeCache             protoimpl.SizeCache
}
...
func (x *SubmitChoicesResponse) GetRemainingRequirements() []*Choice {
	if x != nil {
...
// Deprecated: enum-based fixed slot list. Use dnd5e.api.v1alpha2.encounter.CharacterData
// (slots: repeated SlotDef, server-owned data, no fixed enum) instead. See
// rpg-api-protos#187.
...
// Deprecated: use dnd5e.api.v1alpha2.encounter.Item (Ref-based, display fields
// server-composed) instead. See rpg-api-protos#187.
...
type ListSpellsByLevelResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Spells        []*SpellInfo           `protobuf:"bytes,1,rep,name=spells,proto3" json:"spells,omitempty"`
	NextPageToken string                 `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	TotalSize     int32                  `protobuf:"varint,3,opt,name=total_size,json=totalSize,proto3" json:"total_size,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
...
func (x *ListSpellsByLevelResponse) GetSpells() []*SpellInfo {
	if x != nil {
		return x.Spells
	}
	return nil
...
const file_dnd5e_api_v1alpha1_character_proto_rawDesc = "" +
	"\n" +
	"\"dnd5e/api/v1alpha1/character.proto\x12\x12dnd5e.api.v1alpha1\x1a,dnd5e/api/customization/v1alpha1/types.proto\x1a dnd5e/api/v1alpha1/choices.proto\x1a\x1fdnd5e/api/v1alpha1/common.proto\x1a\x1ednd5e/api/v1alpha1/enums.proto\x1a(dnd5e/api/v1alpha1/equipment_types.proto\"\x89\x02\n" +
	"\x10CharacterFeature\x12-\n" +
	"\x02id\x18\x01 \x01(\x0e2\x1d.dnd5e.api.v1alpha1.FeatureIdR\x02id\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12 \n" +
	"\vdescription\x18\x03 \x01(\tR\vdescription\x12\x16\n" +
	"\x06source\x18\x04 \x01(\tR\x06source\x12\x14\n" +
	"\x05level\x18\x05 \x01(\x05R\x05level\x12?\n" +
	"\vaction_type\x18\x06 \x01(\x0e2\x1e.dnd5e.api.v1alpha1.ActionTypeR\n" +
	"actionType\x12!\n" +
	"\ffeature_data\x18\a \x01(\fR\vfeatureData\"\xd0\x01\n" +
	"\rClassResource\x129\n" +
	...
	"\alevel_9\x18\t \x01(\x05R\x06level9\"\xf0\x0e\n" +
	"\tCharacter\x12\x0e\n" +
	...
	"\x0fclass_resources\x18\x1a \x03(\v2!.dnd5e.api.v1alpha1.ClassResourceR\x0eclassResources\x12C\n" +
	"\vspell_slots\x18\x1b \x01(\v2\x1e.dnd5e.api.v1alpha1.SpellSlotsB\x02\x18\x01R\n" +
	"spellSlots\x12J\n" +
	"\x11active_conditions\x18\x1c \x03(\v2\x1d.dnd5e.api.v1alpha1.ConditionR\x10activeConditions\x12>\n" +
	"\n" +
	"appearance\x18\x1d \x01(\v2\x1e.dnd5e.api.v1alpha1.AppearanceR\n" +
	"appearance\x12%\n" +
	"\x0eknown_cantrips\x18\x1e \x03(\tR\rknownCantrips\x12!\n" +
	"\fknown_spells\x18\x1f \x03(\tR\vknownSpells\x12%\n" +
	"\x0eentitled_level\x18  \x01(\x05R\rentitledLevel\x120\n" +
	"\x14next_level_threshold\x18! \x01(\x05R\x12nextLevelThreshold\"\xbc\x02\n" +
	"\n" +
	"Appearance\x12\x1f\n" +
	"\tskin_tone\x18\x01 \x01(\tB\x02\x18\x01R\bskinTone\x12'\n" +
	"\rprimary_color\x18\x02 \x01(\tB\x02\x18\x01R\fprimaryColor\x12+\n" +
	"\x0fsecondary_color\x18\x03 \x01(\tB\x02\x18\x01R\x0esecondaryColor\x12\x1f\n" +
	"\teye_color\x18\x04 \x01(\tB\x02\x18\x01R\beyeColor\x12G\n" +
	"\x04hair\x18\x05 \x01(\v23.dnd5e.api.customization.v1alpha1.HairCustomizationR\x04hair\x12M\n" +
	"\x06outfit\x18\x06 \x01(\v25.dnd5e.api.customization.v1alpha1.OutfitCustomizationR\x06outfit\"\xc8\x01\n" +
	...
	"\tlanguages\x18\x05 \x03(\x0e2\x1c.dnd5e.api.v1alpha1.LanguageR\tlanguages\"\xe5\x04\n" +
	"\rProficiencies\x12D\n" +
	"\x10expertise_skills\x18\t \x03(\x0e2\x19.dnd5e.api.v1alpha1.SkillR\x0fexpertiseSkills\x121\n" +
	"\x06skills\x18\x01 \x03(\x0e2\x19.dnd5e.api.v1alpha1.SkillR\x06skills\x12@\n" +
	"\rsaving_throws\x18\x02 \x03(\x0e2\x1b.dnd5e.api.v1alpha1.AbilityR\fsavingThrows\x12W\n" +
	"\x10armor_categories\x18\x06 \x03(\x0e2,.dnd5e.api.v1alpha1.ArmorProficiencyCategoryR\x0farmorCategories\x12Z\n" +
	"\x11weapon_categories\x18\a \x03(\x0e2-.dnd5e.api.v1alpha1.WeaponProficiencyCategoryR\x10weaponCategories\x12E\n" +
	"\x10specific_weapons\x18\b \x03(\x0e2\x1a.dnd5e.api.v1alpha1.WeaponR\x0fspecificWeapons\x12.\n" +
	"\x05tools\x18\x05 \x03(\x0e2\x18.dnd5e.api.v1alpha1.ToolR\x05tools\x123\n" +
	"\x05armor\x18\x03 \x03(\x0e2\x19.dnd5e.api.v1alpha1.ArmorB\x02\x18\x01R\x05armor\x128\n" +
	"\aweapons\x18\x04 \x03(\x0e2\x1a.dnd5e.api.v1alpha1.WeaponB\x02\x18\x01R\aweapons\"n\n" +
	...
	"\x13GetNextLevelRequest\x12!\n" +
	"\fcharacter_id\x18\x01 \x01(\tR\vcharacterId\"\xe9\x01\n" +
	"\x14GetNextLevelResponse\x12\x14\n" +
	"\x05level\x18\x01 \x01(\x05R\x05level\x12/\n" +
	"\x05class\x18\x02 \x01(\x0e2\x19.dnd5e.api.v1alpha1.ClassR\x05class\x124\n" +
	"\achoices\x18\x03 \x03(\v2\x1a.dnd5e.api.v1alpha1.ChoiceR\achoices\x12;\n" +
	"\bfeatures\x18\x04 \x03(\v2\x1f.dnd5e.api.v1alpha1.FeatureInfoR\bfeatures\x12\x17\n" +
	"\ahit_die\x18\x05 \x01(\x05R\x06hitDie\"\xbb\x01\n" +
	"\x0eLevelUpRequest\x12!\n" +
	"\fcharacter_id\x18\x01 \x01(\tR\vcharacterId\x12L\n" +
	"\x10hit_point_method\x18\x02 \x01(\x0e2\".dnd5e.api.v1alpha1.HitPointMethodR\x0ehitPointMethod\x128\n" +
	"\achoices\x18\x03 \x03(\v2\x1e.dnd5e.api.v1alpha1.ChoiceDataR\achoices\"\x87\x01\n" +
	"\x0fLevelUpResponse\x12;\n" +
	"\tcharacter\x18\x01 \x01(\v2\x1d.dnd5e.api.v1alpha1.CharacterR\tcharacter\x127\n" +
	"\x06gained\x18\x02 \x01(\v2\x1f.dnd5e.api.v1alpha1.LevelGainedR\x06gained\"\xe2\x01\n" +
	"\vLevelGained\x12\x14\n" +
	"\x05level\x18\x01 \x01(\x05R\x05level\x12*\n" +
	"\x11hit_points_gained\x18\x02 \x01(\x05R\x0fhitPointsGained\x12;\n" +
	"\bfeatures\x18\x03 \x03(\v2\x1f.dnd5e.api.v1alpha1.FeatureInfoR\bfeatures\x12T\n" +
	"\x10resource_changes\x18\x04 \x03(\v2).dnd5e.api.v1alpha1.ResourceMaximumChangeR\x0fresourceChanges\"\x89\x01\n" +
	"\x15ResourceMaximumChange\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12)\n" +
	"\x10previous_maximum\x18\x03 \x01(\x05R\x0fpreviousMaximum\x12\x1f\n" +
	"\vnew_maximum\x18\x04 \x01(\x05R\n" +
	"newMaximum\"\x93\x05\n" +
	...
	"\bprogress\x18\f \x01(\v2$.dnd5e.api.v1alpha1.CreationProgressR\bprogress\x12>\n" +
	"\n" +
	"appearance\x18\r \x01(\v2\x1e.dnd5e.api.v1alpha1.AppearanceR\n" +
	"appearance\"\xef\x01\n" +
	...
	"\x12has_ability_scores\x18\x05 \x01(\bR\x10hasAbilityScores\x123\n" +
	"\x15completion_percentage\x18\x06 \x01(\x05R\x14completionPercentage\"\xa9\x01\n" +
	...
	"\x12discord_message_id\x18\x04 \x01(\tR\x10discordMessageId\"\xdf\a\n" +
	...
	"\x0fbackground_info\x18\x11 \x01(\v2\".dnd5e.api.v1alpha1.BackgroundInfoR\x0ebackgroundInfo\x12>\n" +
	"\n" +
	"appearance\x18\x12 \x01(\v2\x1e.dnd5e.api.v1alpha1.AppearanceR\n" +
	"appearance\"\x9b\x01\n" +
	...
	"\x05draft\x18\x01 \x01(\v2\".dnd5e.api.v1alpha1.CharacterDraftR\x05draft\"B\n" +
	...
	"\x06skills\x18\x02 \x03(\x0e2\x19.dnd5e.api.v1alpha1.SkillR\x06skills\"N\n" +
	"\x12UpdateNameResponse\x128\n" +
	"\x05draft\x18\x01 \x01(\v2\".dnd5e.api.v1alpha1.CharacterDraftR\x05draft\"N\n" +
	"\x12UpdateRaceResponse\x128\n" +
	"\x05draft\x18\x01 \x01(\v2\".dnd5e.api.v1alpha1.CharacterDraftR\x05draft\"O\n" +
	"\x13UpdateClassResponse\x128\n" +
	"\x05draft\x18\x01 \x01(\v2\".dnd5e.api.v1alpha1.CharacterDraftR\x05draft\"T\n" +
	"\x18UpdateBackgroundResponse\x128\n" +
	"\x05draft\x18\x01 \x01(\v2\".dnd5e.api.v1alpha1.CharacterDraftR\x05draft\"W\n" +
	"\x1bUpdateAbilityScoresResponse\x128\n" +
	"\x05draft\x18\x01 \x01(\v2\".dnd5e.api.v1alpha1.CharacterDraftR\x05draft\"P\n" +
	"\x14UpdateSkillsResponse\x128\n" +
	"\x05draft\x18\x01 \x01(\v2\".dnd5e.api.v1alpha1.CharacterDraftR\x05draft\"t\n" +
	"\x17UpdateAppearanceRequest\x12\x19\n" +
	"\bdraft_id\x18\x01 \x01(\tR\adraftId\x12>\n" +
	"\n" +
	"appearance\x18\x02 \x01(\v2\x1e.dnd5e.api.v1alpha1.AppearanceR\n" +
	"appearance\"T\n" +
	"\x18UpdateAppearanceResponse\x128\n" +
	"\x05draft\x18\x01 \x01(\v2\".dnd5e.api.v1alpha1.CharacterDraftR\x05draft\"\x8b\x01\n" +
	...
	"\bdraft_id\x18\x01 \x01(\tR\adraftId\"\x99\x01\n" +
	...
	"\bdraft_id\x18\x01 \x01(\tR\adraftId\"\x8c\x01\n" +
	"\x17GetDraftPreviewResponse\x128\n" +
	"\x05draft\x18\x01 \x01(\v2\".dnd5e.api.v1alpha1.CharacterDraftR\x05draft\x127\n" +
	"\apreview\x18\x02 \x01(\v2\x1d.dnd5e.api.v1alpha1.CharacterR\apreview\"1\n" +
	...
	"\amessage\x18\x01 \x01(\tR\amessage\"\x93\x05\n" +
	"\bRaceInfo\x121\n" +
	...
	"\achoices\x18\v \x03(\v2\x1a.dnd5e.api.v1alpha1.ChoiceR\achoices\x1aA\n" +
	"\x13AbilityBonusesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\x05R\x05value:\x028\x01\"\xeb\x03\n" +
	"\vSubraceInfo\x12:\n" +
	"\n" +
	...
	"\x12proficiency_grants\x18\a \x01(\v2%.dnd5e.api.v1alpha1.ProficiencyGrantsR\x11proficiencyGrants\x1aA\n" +
	"\x13AbilityBonusesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	...
	"\aoptions\x18\x04 \x03(\tR\aoptions\"\xca\a\n" +
	"\tClassInfo\x124\n" +
	...
	"\x1dspecific_weapon_proficiencies\x18\b \x03(\x0e2\x1a.dnd5e.api.v1alpha1.WeaponR\x1bspecificWeaponProficiencies\x12G\n" +
	"\x12tool_proficiencies\x18\t \x03(\x0e2\x18.dnd5e.api.v1alpha1.ToolR\x11toolProficiencies\x12Y\n" +
	"\x1asaving_throw_proficiencies\x18\n" +
	" \x03(\x0e2\x1b.dnd5e.api.v1alpha1.AbilityR\x18savingThrowProficiencies\x12I\n" +
	"\x10level_1_features\x18\v \x03(\v2\x1f.dnd5e.api.v1alpha1.FeatureInfoR\x0elevel1Features\x12H\n" +
	"\fspellcasting\x18\f \x01(\v2$.dnd5e.api.v1alpha1.SpellcastingInfoR\fspellcasting\x124\n" +
	"\achoices\x18\r \x03(\v2\x1a.dnd5e.api.v1alpha1.ChoiceR\achoices\x12@\n" +
	"\n" +
	"subclasses\x18\x0e \x03(\v2 .dnd5e.api.v1alpha1.SubclassInfoR\n" +
	"subclasses\"\x85\x04\n" +
	"\fSubclassInfo\x12=\n" +
	...
	"\bfeatures\x18\t \x03(\v2\x1f.dnd5e.api.v1alpha1.FeatureInfoR\bfeatures\"\x94\x02\n" +
	...
	"\x13spell_slots_level_1\x18\x06 \x01(\x05R\x10spellSlotsLevel1\"\xda\x01\n" +
	"\tSpellInfo\x124\n" +
	"\bspell_id\x18\x01 \x01(\x0e2\x19.dnd5e.api.v1alpha1.SpellR\aspellId\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12 \n" +
	"\vdescription\x18\x03 \x01(\tR\vdescription\x12\x14\n" +
	"\x05level\x18\x04 \x01(\x05R\x05level\x12\x1b\n" +
	"\tspell_ref\x18\x05 \x01(\tR\bspellRef\x12.\n" +
	"\x13not_yet_implemented\x18\x06 \x01(\bR\x11notYetImplemented\"\x88\x06\n" +
	...
	"\x05flaws\x18\x0f \x03(\tR\x05flaws\x124\n" +
	"\achoices\x18\x10 \x03(\v2\x1a.dnd5e.api.v1alpha1.ChoiceR\achoices\"\xb0\x02\n" +
	...
	"\x04race\x18\x03 \x01(\x0e2\x18.dnd5e.api.v1alpha1.RaceR\x04race\"Y\n" +
	"\x17GetRequirementsResponse\x12>\n" +
	"\frequirements\x18\x01 \x03(\v2\x1a.dnd5e.api.v1alpha1.ChoiceR\frequirements\"y\n" +
	"\x14SubmitChoicesRequest\x12\x19\n" +
	"\bdraft_id\x18\x01 \x01(\tR\adraftId\x12F\n" +
	"\vsubmissions\x18\x02 \x03(\v2$.dnd5e.api.v1alpha1.ChoiceSubmissionR\vsubmissions\"\xea\x01\n" +
	...
	"validation\x12Q\n" +
	"\x16remaining_requirements\x18\x03 \x03(\v2\x1a.dnd5e.api.v1alpha1.ChoiceR\x15remainingRequirements\"\x9b\x02\n" +
	...
	"\x10charisma_roll_id\x18\x06 \x01(\tR\x0echarismaRollId\"\x95\x05\n" +
	...
	"\x05level\x18\x04 \x01(\x0e2$.dnd5e.api.v1alpha1.EncumbranceLevelR\x05level\"\xa6\x01\n" +
	"\vSpellDamage\x12?\n" +
	...
	"\x19ListSpellsByLevelResponse\x125\n" +
	"\x06spells\x18\x01 \x03(\v2\x1d.dnd5e.api.v1alpha1.SpellInfoR\x06spells\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\x12\x1d\n" +
	"\n" +
	...
	"\x12RECHARGE_TYPE_NONE\x10\x04*m\n" +
	"\x0eHitPointMethod\x12 \n" +
	"\x1cHIT_POINT_METHOD_UNSPECIFIED\x10\x00\x12\x1b\n" +
	"\x17HIT_POINT_METHOD_ROLLED\x10\x01\x12\x1c\n" +
	"\x18HIT_POINT_METHOD_AVERAGE\x10\x02*\x98\x01\n" +
	...
	"\x0fSIZE_GARGANTUAN\x10\x06*\xc8\x01\n" +
	...
	"\x1dENCUMBRANCE_LEVEL_IMMOBILIZED\x10\x04*\xd8\x02\n" +
	...
	"\x12\x17\n" +
	"\x13EQUIPMENT_SLOT_BELT\x10\v2\x9a\x1d\n" +
	...
	"\fUpdateSkills\x12'.dnd5e.api.v1alpha1.UpdateSkillsRequest\x1a(.dnd5e.api.v1alpha1.UpdateSkillsResponse\x12m\n" +
	"\x10UpdateAppearance\x12+.dnd5e.api.v1alpha1.UpdateAppearanceRequest\x1a,.dnd5e.api.v1alpha1.UpdateAppearanceResponse\x12d\n" +
	...
	"\x0fDeleteCharacter\x12*.dnd5e.api.v1alpha1.DeleteCharacterRequest\x1a+.dnd5e.api.v1alpha1.DeleteCharacterResponse\x12a\n" +
	"\fGetNextLevel\x12'.dnd5e.api.v1alpha1.GetNextLevelRequest\x1a(.dnd5e.api.v1alpha1.GetNextLevelResponse\x12R\n" +
	"\aLevelUp\x12\".dnd5e.api.v1alpha1.LevelUpRequest\x1a#.dnd5e.api.v1alpha1.LevelUpResponse\x12X\n" +
...
var file_dnd5e_api_v1alpha1_character_proto_enumTypes = make([]protoimpl.EnumInfo, 7)
var file_dnd5e_api_v1alpha1_character_proto_msgTypes = make([]protoimpl.MessageInfo, 113)
var file_dnd5e_api_v1alpha1_character_proto_goTypes = []any{
	...
	(HitPointMethod)(0),                   // 2: dnd5e.api.v1alpha1.HitPointMethod
	(WarningType)(0),                      // 3: dnd5e.api.v1alpha1.WarningType
	(Size)(0),                             // 4: dnd5e.api.v1alpha1.Size
	(EncumbranceLevel)(0),                 // 5: dnd5e.api.v1alpha1.EncumbranceLevel
	(EquipmentSlot)(0),                    // 6: dnd5e.api.v1alpha1.EquipmentSlot
	(*CharacterFeature)(nil),              // 7: dnd5e.api.v1alpha1.CharacterFeature
	(*ClassResource)(nil),                 // 8: dnd5e.api.v1alpha1.ClassResource
	(*SpellSlots)(nil),                    // 9: dnd5e.api.v1alpha1.SpellSlots
	(*Character)(nil),                     // 10: dnd5e.api.v1alpha1.Character
	(*Appearance)(nil),                    // 11: dnd5e.api.v1alpha1.Appearance
	(*AbilityModifiers)(nil),              // 12: dnd5e.api.v1alpha1.AbilityModifiers
	(*CombatStats)(nil),                   // 13: dnd5e.api.v1alpha1.CombatStats
	(*ProficiencyGrants)(nil),             // 14: dnd5e.api.v1alpha1.ProficiencyGrants
	(*Proficiencies)(nil),                 // 15: dnd5e.api.v1alpha1.Proficiencies
	(*CharacterMetadata)(nil),             // 16: dnd5e.api.v1alpha1.CharacterMetadata
	(*GetCharacterRequest)(nil),           // 17: dnd5e.api.v1alpha1.GetCharacterRequest
	(*GetCharacterResponse)(nil),          // 18: dnd5e.api.v1alpha1.GetCharacterResponse
	(*ListCharactersRequest)(nil),         // 19: dnd5e.api.v1alpha1.ListCharactersRequest
	(*ListCharactersResponse)(nil),        // 20: dnd5e.api.v1alpha1.ListCharactersResponse
	(*UpdateCharacterRequest)(nil),        // 21: dnd5e.api.v1alpha1.UpdateCharacterRequest
	(*UpdateCharacterResponse)(nil),       // 22: dnd5e.api.v1alpha1.UpdateCharacterResponse
	(*DeleteCharacterRequest)(nil),        // 23: dnd5e.api.v1alpha1.DeleteCharacterRequest
	(*DeleteCharacterResponse)(nil),       // 24: dnd5e.api.v1alpha1.DeleteCharacterResponse
	(*GetNextLevelRequest)(nil),           // 25: dnd5e.api.v1alpha1.GetNextLevelRequest
	(*GetNextLevelResponse)(nil),          // 26: dnd5e.api.v1alpha1.GetNextLevelResponse
	(*LevelUpRequest)(nil),                // 27: dnd5e.api.v1alpha1.LevelUpRequest
	(*LevelUpResponse)(nil),               // 28: dnd5e.api.v1alpha1.LevelUpResponse
	(*LevelGained)(nil),                   // 29: dnd5e.api.v1alpha1.LevelGained
	(*ResourceMaximumChange)(nil),         // 30: dnd5e.api.v1alpha1.ResourceMaximumChange
	(*CharacterDraftData)(nil),            // 31: dnd5e.api.v1alpha1.CharacterDraftData
	(*CreationProgress)(nil),              // 32: dnd5e.api.v1alpha1.CreationProgress
	(*DraftMetadata)(nil),                 // 33: dnd5e.api.v1alpha1.DraftMetadata
	(*CharacterDraft)(nil),                // 34: dnd5e.api.v1alpha1.CharacterDraft
	(*CreateDraftRequest)(nil),            // 35: dnd5e.api.v1alpha1.CreateDraftRequest
	(*CreateDraftResponse)(nil),           // 36: dnd5e.api.v1alpha1.CreateDraftResponse
	(*GetDraftRequest)(nil),               // 37: dnd5e.api.v1alpha1.GetDraftRequest
	(*GetDraftResponse)(nil),              // 38: dnd5e.api.v1alpha1.GetDraftResponse
	(*UpdateDraftRequest)(nil),            // 39: dnd5e.api.v1alpha1.UpdateDraftRequest
	(*UpdateDraftResponse)(nil),           // 40: dnd5e.api.v1alpha1.UpdateDraftResponse
	(*UpdateNameRequest)(nil),             // 41: dnd5e.api.v1alpha1.UpdateNameRequest
	(*UpdateRaceRequest)(nil),             // 42: dnd5e.api.v1alpha1.UpdateRaceRequest
	(*UpdateClassRequest)(nil),            // 43: dnd5e.api.v1alpha1.UpdateClassRequest
	(*UpdateBackgroundRequest)(nil),       // 44: dnd5e.api.v1alpha1.UpdateBackgroundRequest
	(*UpdateAbilityScoresRequest)(nil),    // 45: dnd5e.api.v1alpha1.UpdateAbilityScoresRequest
	(*UpdateSkillsRequest)(nil),           // 46: dnd5e.api.v1alpha1.UpdateSkillsRequest
	(*UpdateNameResponse)(nil),            // 47: dnd5e.api.v1alpha1.UpdateNameResponse
	(*UpdateRaceResponse)(nil),            // 48: dnd5e.api.v1alpha1.UpdateRaceResponse
	(*UpdateClassResponse)(nil),           // 49: dnd5e.api.v1alpha1.UpdateClassResponse
	(*UpdateBackgroundResponse)(nil),      // 50: dnd5e.api.v1alpha1.UpdateBackgroundResponse
	(*UpdateAbilityScoresResponse)(nil),   // 51: dnd5e.api.v1alpha1.UpdateAbilityScoresResponse
	(*UpdateSkillsResponse)(nil),          // 52: dnd5e.api.v1alpha1.UpdateSkillsResponse
	(*UpdateAppearanceRequest)(nil),       // 53: dnd5e.api.v1alpha1.UpdateAppearanceRequest
	(*UpdateAppearanceResponse)(nil),      // 54: dnd5e.api.v1alpha1.UpdateAppearanceResponse
	(*ListDraftsRequest)(nil),             // 55: dnd5e.api.v1alpha1.ListDraftsRequest
	(*ListDraftsResponse)(nil),            // 56: dnd5e.api.v1alpha1.ListDraftsResponse
	(*ValidateDraftRequest)(nil),          // 57: dnd5e.api.v1alpha1.ValidateDraftRequest
	(*ValidateDraftResponse)(nil),         // 58: dnd5e.api.v1alpha1.ValidateDraftResponse
	(*GetDraftPreviewRequest)(nil),        // 59: dnd5e.api.v1alpha1.GetDraftPreviewRequest
	(*GetDraftPreviewResponse)(nil),       // 60: dnd5e.api.v1alpha1.GetDraftPreviewResponse
	(*FinalizeDraftRequest)(nil),          // 61: dnd5e.api.v1alpha1.FinalizeDraftRequest
	(*FinalizeDraftResponse)(nil),         // 62: dnd5e.api.v1alpha1.FinalizeDraftResponse
	(*DeleteDraftRequest)(nil),            // 63: dnd5e.api.v1alpha1.DeleteDraftRequest
	(*DeleteDraftResponse)(nil),           // 64: dnd5e.api.v1alpha1.DeleteDraftResponse
	(*RaceInfo)(nil),                      // 65: dnd5e.api.v1alpha1.RaceInfo
	(*SubraceInfo)(nil),                   // 66: dnd5e.api.v1alpha1.SubraceInfo
	(*RacialTrait)(nil),                   // 67: dnd5e.api.v1alpha1.RacialTrait
	(*ClassInfo)(nil),                     // 68: dnd5e.api.v1alpha1.ClassInfo
	(*SubclassInfo)(nil),                  // 69: dnd5e.api.v1alpha1.SubclassInfo
	(*SpellcastingInfo)(nil),              // 70: dnd5e.api.v1alpha1.SpellcastingInfo
	(*SpellInfo)(nil),                     // 71: dnd5e.api.v1alpha1.SpellInfo
	(*BackgroundInfo)(nil),                // 72: dnd5e.api.v1alpha1.BackgroundInfo
	(*FeatureInfo)(nil),                   // 73: dnd5e.api.v1alpha1.FeatureInfo
	(*SpellSelectionInfo)(nil),            // 74: dnd5e.api.v1alpha1.SpellSelectionInfo
	(*ListRacesRequest)(nil),              // 75: dnd5e.api.v1alpha1.ListRacesRequest
	(*ListRacesResponse)(nil),             // 76: dnd5e.api.v1alpha1.ListRacesResponse
	(*ListClassesRequest)(nil),            // 77: dnd5e.api.v1alpha1.ListClassesRequest
	(*ListClassesResponse)(nil),           // 78: dnd5e.api.v1alpha1.ListClassesResponse
	(*ListBackgroundsRequest)(nil),        // 79: dnd5e.api.v1alpha1.ListBackgroundsRequest
	(*ListBackgroundsResponse)(nil),       // 80: dnd5e.api.v1alpha1.ListBackgroundsResponse
	(*GetRaceDetailsRequest)(nil),         // 81: dnd5e.api.v1alpha1.GetRaceDetailsRequest
	(*GetRaceDetailsResponse)(nil),        // 82: dnd5e.api.v1alpha1.GetRaceDetailsResponse
	(*GetClassDetailsRequest)(nil),        // 83: dnd5e.api.v1alpha1.GetClassDetailsRequest
	(*GetClassDetailsResponse)(nil),       // 84: dnd5e.api.v1alpha1.GetClassDetailsResponse
	(*GetBackgroundDetailsRequest)(nil),   // 85: dnd5e.api.v1alpha1.GetBackgroundDetailsRequest
	(*GetBackgroundDetailsResponse)(nil),  // 86: dnd5e.api.v1alpha1.GetBackgroundDetailsResponse
	(*GetFeatureRequest)(nil),             // 87: dnd5e.api.v1alpha1.GetFeatureRequest
	(*GetFeatureResponse)(nil),            // 88: dnd5e.api.v1alpha1.GetFeatureResponse
	(*RollAbilityScoresRequest)(nil),      // 89: dnd5e.api.v1alpha1.RollAbilityScoresRequest
	(*AbilityScoreRoll)(nil),              // 90: dnd5e.api.v1alpha1.AbilityScoreRoll
	(*RollAbilityScoresResponse)(nil),     // 91: dnd5e.api.v1alpha1.RollAbilityScoresResponse
	(*GetRequirementsRequest)(nil),        // 92: dnd5e.api.v1alpha1.GetRequirementsRequest
	(*GetRequirementsResponse)(nil),       // 93: dnd5e.api.v1alpha1.GetRequirementsResponse
	(*SubmitChoicesRequest)(nil),          // 94: dnd5e.api.v1alpha1.SubmitChoicesRequest
	(*SubmitChoicesResponse)(nil),         // 95: dnd5e.api.v1alpha1.SubmitChoicesResponse
	(*RollAssignments)(nil),               // 96: dnd5e.api.v1alpha1.RollAssignments
	(*EquipmentSlots)(nil),                // 97: dnd5e.api.v1alpha1.EquipmentSlots
	(*InventoryItem)(nil),                 // 98: dnd5e.api.v1alpha1.InventoryItem
	(*EncumbranceInfo)(nil),               // 99: dnd5e.api.v1alpha1.EncumbranceInfo
	(*SpellDamage)(nil),                   // 100: dnd5e.api.v1alpha1.SpellDamage
	(*DamageAtSlotLevel)(nil),             // 101: dnd5e.api.v1alpha1.DamageAtSlotLevel
	(*AreaOfEffect)(nil),                  // 102: dnd5e.api.v1alpha1.AreaOfEffect
	(*ListEquipmentByTypeRequest)(nil),    // 103: dnd5e.api.v1alpha1.ListEquipmentByTypeRequest
	(*ListEquipmentByTypeResponse)(nil),   // 104: dnd5e.api.v1alpha1.ListEquipmentByTypeResponse
	(*ListSpellsByLevelRequest)(nil),      // 105: dnd5e.api.v1alpha1.ListSpellsByLevelRequest
	(*ListSpellsByLevelResponse)(nil),     // 106: dnd5e.api.v1alpha1.ListSpellsByLevelResponse
	(*GetCharacterInventoryRequest)(nil),  // 107: dnd5e.api.v1alpha1.GetCharacterInventoryRequest
	(*GetCharacterInventoryResponse)(nil), // 108: dnd5e.api.v1alpha1.GetCharacterInventoryResponse
	(*EquipItemRequest)(nil),              // 109: dnd5e.api.v1alpha1.EquipItemRequest
	(*EquipItemResponse)(nil),             // 110: dnd5e.api.v1alpha1.EquipItemResponse
	(*UnequipItemRequest)(nil),            // 111: dnd5e.api.v1alpha1.UnequipItemRequest
	(*UnequipItemResponse)(nil),           // 112: dnd5e.api.v1alpha1.UnequipItemResponse
	(*AddToInventoryRequest)(nil),         // 113: dnd5e.api.v1alpha1.AddToInventoryRequest
	(*InventoryAddition)(nil),             // 114: dnd5e.api.v1alpha1.InventoryAddition
	(*AddToInventoryResponse)(nil),        // 115: dnd5e.api.v1alpha1.AddToInventoryResponse
	(*RemoveFromInventoryRequest)(nil),    // 116: dnd5e.api.v1alpha1.RemoveFromInventoryRequest
	(*RemoveFromInventoryResponse)(nil),   // 117: dnd5e.api.v1alpha1.RemoveFromInventoryResponse
	nil,                                   // 118: dnd5e.api.v1alpha1.RaceInfo.AbilityBonusesEntry
	nil,                                   // 119: dnd5e.api.v1alpha1.SubraceInfo.AbilityBonusesEntry
	(FeatureId)(0),                        // 120: dnd5e.api.v1alpha1.FeatureId
	(ActionType)(0),                       // 121: dnd5e.api.v1alpha1.ActionType
	(Race)(0),                             // 122: dnd5e.api.v1alpha1.Race
	(Subrace)(0),                          // 123: dnd5e.api.v1alpha1.Subrace
	(Class)(0),                            // 124: dnd5e.api.v1alpha1.Class
	(Background)(0),                       // 125: dnd5e.api.v1alpha1.Background
	(Alignment)(0),                        // 126: dnd5e.api.v1alpha1.Alignment
	(*AbilityScores)(nil),                 // 127: dnd5e.api.v1alpha1.AbilityScores
	(Language)(0),                         // 128: dnd5e.api.v1alpha1.Language
	(FightingStyle)(0),                    // 129: dnd5e.api.v1alpha1.FightingStyle
	(*Condition)(nil),                     // 130: dnd5e.api.v1alpha1.Condition
	(*v1alpha1.HairCustomization)(nil),    // 131: dnd5e.api.customization.v1alpha1.HairCustomization
	(*v1alpha1.OutfitCustomization)(nil),  // 132: dnd5e.api.customization.v1alpha1.OutfitCustomization
	(Armor)(0),                            // 133: dnd5e.api.v1alpha1.Armor
	(Weapon)(0),                           // 134: dnd5e.api.v1alpha1.Weapon
	(Tool)(0),                             // 135: dnd5e.api.v1alpha1.Tool
	(Skill)(0),                            // 136: dnd5e.api.v1alpha1.Skill
	(Ability)(0),                          // 137: dnd5e.api.v1alpha1.Ability
	(ArmorProficiencyCategory)(0),         // 138: dnd5e.api.v1alpha1.ArmorProficiencyCategory
	(WeaponProficiencyCategory)(0),        // 139: dnd5e.api.v1alpha1.WeaponProficiencyCategory
	(*Choice)(nil),                        // 140: dnd5e.api.v1alpha1.Choice
	(*ChoiceData)(nil),                    // 141: dnd5e.api.v1alpha1.ChoiceData
	(Subclass)(0),                         // 142: dnd5e.api.v1alpha1.Subclass
	(*ValidationResult)(nil),              // 143: dnd5e.api.v1alpha1.ValidationResult
	(Spell)(0),                            // 144: dnd5e.api.v1alpha1.Spell
	(*EquipmentSelection)(nil),            // 145: dnd5e.api.v1alpha1.EquipmentSelection
	(SpellSelectionType)(0),               // 146: dnd5e.api.v1alpha1.SpellSelectionType
	(*ChoiceSubmission)(nil),              // 147: dnd5e.api.v1alpha1.ChoiceSubmission
	(*Equipment)(nil),                     // 148: dnd5e.api.v1alpha1.Equipment
	(DamageType)(0),                       // 149: dnd5e.api.v1alpha1.DamageType
	(EquipmentType)(0),                    // 150: dnd5e.api.v1alpha1.EquipmentType
}
var file_dnd5e_api_v1alpha1_character_proto_depIdxs = []int32{
	120, // 0: dnd5e.api.v1alpha1.CharacterFeature.id:type_name -> dnd5e.api.v1alpha1.FeatureId
	121, // 1: dnd5e.api.v1alpha1.CharacterFeature.action_type:type_name -> dnd5e.api.v1alpha1.ActionType
	0,   // 2: dnd5e.api.v1alpha1.ClassResource.type:type_name -> dnd5e.api.v1alpha1.ClassResourceType
	1,   // 3: dnd5e.api.v1alpha1.ClassResource.recharge:type_name -> dnd5e.api.v1alpha1.RechargeType
	122, // 4: dnd5e.api.v1alpha1.Character.race:type_name -> dnd5e.api.v1alpha1.Race
	123, // 5: dnd5e.api.v1alpha1.Character.subrace:type_name -> dnd5e.api.v1alpha1.Subrace
	124, // 6: dnd5e.api.v1alpha1.Character.class:type_name -> dnd5e.api.v1alpha1.Class
	125, // 7: dnd5e.api.v1alpha1.Character.background:type_name -> dnd5e.api.v1alpha1.Background
	126, // 8: dnd5e.api.v1alpha1.Character.alignment:type_name -> dnd5e.api.v1alpha1.Alignment
	127, // 9: dnd5e.api.v1alpha1.Character.ability_scores:type_name -> dnd5e.api.v1alpha1.AbilityScores
	12,  // 10: dnd5e.api.v1alpha1.Character.ability_modifiers:type_name -> dnd5e.api.v1alpha1.AbilityModifiers
	13,  // 11: dnd5e.api.v1alpha1.Character.combat_stats:type_name -> dnd5e.api.v1alpha1.CombatStats
	15,  // 12: dnd5e.api.v1alpha1.Character.proficiencies:type_name -> dnd5e.api.v1alpha1.Proficiencies
	128, // 13: dnd5e.api.v1alpha1.Character.languages:type_name -> dnd5e.api.v1alpha1.Language
	16,  // 14: dnd5e.api.v1alpha1.Character.metadata:type_name -> dnd5e.api.v1alpha1.CharacterMetadata
	97,  // 15: dnd5e.api.v1alpha1.Character.equipment_slots:type_name -> dnd5e.api.v1alpha1.EquipmentSlots
	98,  // 16: dnd5e.api.v1alpha1.Character.inventory:type_name -> dnd5e.api.v1alpha1.InventoryItem
	99,  // 17: dnd5e.api.v1alpha1.Character.encumbrance:type_name -> dnd5e.api.v1alpha1.EncumbranceInfo
	7,   // 18: dnd5e.api.v1alpha1.Character.features:type_name -> dnd5e.api.v1alpha1.CharacterFeature
	129, // 19: dnd5e.api.v1alpha1.Character.fighting_styles:type_name -> dnd5e.api.v1alpha1.FightingStyle
	7,   // 20: dnd5e.api.v1alpha1.Character.racial_traits:type_name -> dnd5e.api.v1alpha1.CharacterFeature
	7,   // 21: dnd5e.api.v1alpha1.Character.background_feature:type_name -> dnd5e.api.v1alpha1.CharacterFeature
	8,   // 22: dnd5e.api.v1alpha1.Character.class_resources:type_name -> dnd5e.api.v1alpha1.ClassResource
	9,   // 23: dnd5e.api.v1alpha1.Character.spell_slots:type_name -> dnd5e.api.v1alpha1.SpellSlots
	130, // 24: dnd5e.api.v1alpha1.Character.active_conditions:type_name -> dnd5e.api.v1alpha1.Condition
	11,  // 25: dnd5e.api.v1alpha1.Character.appearance:type_name -> dnd5e.api.v1alpha1.Appearance
	131, // 26: dnd5e.api.v1alpha1.Appearance.hair:type_name -> dnd5e.api.customization.v1alpha1.HairCustomization
	132, // 27: dnd5e.api.v1alpha1.Appearance.outfit:type_name -> dnd5e.api.customization.v1alpha1.OutfitCustomization
	133, // 28: dnd5e.api.v1alpha1.ProficiencyGrants.armor:type_name -> dnd5e.api.v1alpha1.Armor
	134, // 29: dnd5e.api.v1alpha1.ProficiencyGrants.weapons:type_name -> dnd5e.api.v1alpha1.Weapon
	135, // 30: dnd5e.api.v1alpha1.ProficiencyGrants.tools:type_name -> dnd5e.api.v1alpha1.Tool
	136, // 31: dnd5e.api.v1alpha1.ProficiencyGrants.skills:type_name -> dnd5e.api.v1alpha1.Skill
	128, // 32: dnd5e.api.v1alpha1.ProficiencyGrants.languages:type_name -> dnd5e.api.v1alpha1.Language
	136, // 33: dnd5e.api.v1alpha1.Proficiencies.expertise_skills:type_name -> dnd5e.api.v1alpha1.Skill
	136, // 34: dnd5e.api.v1alpha1.Proficiencies.skills:type_name -> dnd5e.api.v1alpha1.Skill
	137, // 35: dnd5e.api.v1alpha1.Proficiencies.saving_throws:type_name -> dnd5e.api.v1alpha1.Ability
	138, // 36: dnd5e.api.v1alpha1.Proficiencies.armor_categories:type_name -> dnd5e.api.v1alpha1.ArmorProficiencyCategory
	139, // 37: dnd5e.api.v1alpha1.Proficiencies.weapon_categories:type_name -> dnd5e.api.v1alpha1.WeaponProficiencyCategory
	134, // 38: dnd5e.api.v1alpha1.Proficiencies.specific_weapons:type_name -> dnd5e.api.v1alpha1.Weapon
	135, // 39: dnd5e.api.v1alpha1.Proficiencies.tools:type_name -> dnd5e.api.v1alpha1.Tool
	133, // 40: dnd5e.api.v1alpha1.Proficiencies.armor:type_name -> dnd5e.api.v1alpha1.Armor
	134, // 41: dnd5e.api.v1alpha1.Proficiencies.weapons:type_name -> dnd5e.api.v1alpha1.Weapon
	10,  // 42: dnd5e.api.v1alpha1.GetCharacterResponse.character:type_name -> dnd5e.api.v1alpha1.Character
	10,  // 43: dnd5e.api.v1alpha1.ListCharactersResponse.characters:type_name -> dnd5e.api.v1alpha1.Character
	10,  // 44: dnd5e.api.v1alpha1.UpdateCharacterRequest.character:type_name -> dnd5e.api.v1alpha1.Character
	10,  // 45: dnd5e.api.v1alpha1.UpdateCharacterResponse.character:type_name -> dnd5e.api.v1alpha1.Character
	124, // 46: dnd5e.api.v1alpha1.GetNextLevelResponse.class:type_name -> dnd5e.api.v1alpha1.Class
	140, // 47: dnd5e.api.v1alpha1.GetNextLevelResponse.choices:type_name -> dnd5e.api.v1alpha1.Choice
	73,  // 48: dnd5e.api.v1alpha1.GetNextLevelResponse.features:type_name -> dnd5e.api.v1alpha1.FeatureInfo
	2,   // 49: dnd5e.api.v1alpha1.LevelUpRequest.hit_point_method:type_name -> dnd5e.api.v1alpha1.HitPointMethod
	141, // 50: dnd5e.api.v1alpha1.LevelUpRequest.choices:type_name -> dnd5e.api.v1alpha1.ChoiceData
	10,  // 51: dnd5e.api.v1alpha1.LevelUpResponse.character:type_name -> dnd5e.api.v1alpha1.Character
	29,  // 52: dnd5e.api.v1alpha1.LevelUpResponse.gained:type_name -> dnd5e.api.v1alpha1.LevelGained
	73,  // 53: dnd5e.api.v1alpha1.LevelGained.features:type_name -> dnd5e.api.v1alpha1.FeatureInfo
	30,  // 54: dnd5e.api.v1alpha1.LevelGained.resource_changes:type_name -> dnd5e.api.v1alpha1.ResourceMaximumChange
	122, // 55: dnd5e.api.v1alpha1.CharacterDraftData.race:type_name -> dnd5e.api.v1alpha1.Race
	123, // 56: dnd5e.api.v1alpha1.CharacterDraftData.subrace:type_name -> dnd5e.api.v1alpha1.Subrace
	124, // 57: dnd5e.api.v1alpha1.CharacterDraftData.class:type_name -> dnd5e.api.v1alpha1.Class
	142, // 58: dnd5e.api.v1alpha1.CharacterDraftData.subclass:type_name -> dnd5e.api.v1alpha1.Subclass
	125, // 59: dnd5e.api.v1alpha1.CharacterDraftData.background:type_name -> dnd5e.api.v1alpha1.Background
	127, // 60: dnd5e.api.v1alpha1.CharacterDraftData.base_ability_scores:type_name -> dnd5e.api.v1alpha1.AbilityScores
	141, // 61: dnd5e.api.v1alpha1.CharacterDraftData.choices:type_name -> dnd5e.api.v1alpha1.ChoiceData
	32,  // 62: dnd5e.api.v1alpha1.CharacterDraftData.progress:type_name -> dnd5e.api.v1alpha1.CreationProgress
	11,  // 63: dnd5e.api.v1alpha1.CharacterDraftData.appearance:type_name -> dnd5e.api.v1alpha1.Appearance
	122, // 64: dnd5e.api.v1alpha1.CharacterDraft.race:type_name -> dnd5e.api.v1alpha1.Race
	123, // 65: dnd5e.api.v1alpha1.CharacterDraft.subrace:type_name -> dnd5e.api.v1alpha1.Subrace
	124, // 66: dnd5e.api.v1alpha1.CharacterDraft.class:type_name -> dnd5e.api.v1alpha1.Class
	142, // 67: dnd5e.api.v1alpha1.CharacterDraft.subclass:type_name -> dnd5e.api.v1alpha1.Subclass
	125, // 68: dnd5e.api.v1alpha1.CharacterDraft.background:type_name -> dnd5e.api.v1alpha1.Background
	127, // 69: dnd5e.api.v1alpha1.CharacterDraft.base_ability_scores:type_name -> dnd5e.api.v1alpha1.AbilityScores
	141, // 70: dnd5e.api.v1alpha1.CharacterDraft.choices:type_name -> dnd5e.api.v1alpha1.ChoiceData
	32,  // 71: dnd5e.api.v1alpha1.CharacterDraft.progress:type_name -> dnd5e.api.v1alpha1.CreationProgress
	143, // 72: dnd5e.api.v1alpha1.CharacterDraft.validation:type_name -> dnd5e.api.v1alpha1.ValidationResult
	65,  // 73: dnd5e.api.v1alpha1.CharacterDraft.race_info:type_name -> dnd5e.api.v1alpha1.RaceInfo
	66,  // 74: dnd5e.api.v1alpha1.CharacterDraft.subrace_info:type_name -> dnd5e.api.v1alpha1.SubraceInfo
	68,  // 75: dnd5e.api.v1alpha1.CharacterDraft.class_info:type_name -> dnd5e.api.v1alpha1.ClassInfo
	72,  // 76: dnd5e.api.v1alpha1.CharacterDraft.background_info:type_name -> dnd5e.api.v1alpha1.BackgroundInfo
	11,  // 77: dnd5e.api.v1alpha1.CharacterDraft.appearance:type_name -> dnd5e.api.v1alpha1.Appearance
	31,  // 78: dnd5e.api.v1alpha1.CreateDraftRequest.initial_data:type_name -> dnd5e.api.v1alpha1.CharacterDraftData
	34,  // 79: dnd5e.api.v1alpha1.CreateDraftResponse.draft:type_name -> dnd5e.api.v1alpha1.CharacterDraft
	34,  // 80: dnd5e.api.v1alpha1.GetDraftResponse.draft:type_name -> dnd5e.api.v1alpha1.CharacterDraft
	31,  // 81: dnd5e.api.v1alpha1.UpdateDraftRequest.updates:type_name -> dnd5e.api.v1alpha1.CharacterDraftData
	34,  // 82: dnd5e.api.v1alpha1.UpdateDraftResponse.draft:type_name -> dnd5e.api.v1alpha1.CharacterDraft
	122, // 83: dnd5e.api.v1alpha1.UpdateRaceRequest.race:type_name -> dnd5e.api.v1alpha1.Race
	123, // 84: dnd5e.api.v1alpha1.UpdateRaceRequest.subrace:type_name -> dnd5e.api.v1alpha1.Subrace
	141, // 85: dnd5e.api.v1alpha1.UpdateRaceRequest.race_choices:type_name -> dnd5e.api.v1alpha1.ChoiceData
	124, // 86: dnd5e.api.v1alpha1.UpdateClassRequest.class:type_name -> dnd5e.api.v1alpha1.Class
	141, // 87: dnd5e.api.v1alpha1.UpdateClassRequest.class_choices:type_name -> dnd5e.api.v1alpha1.ChoiceData
	142, // 88: dnd5e.api.v1alpha1.UpdateClassRequest.subclass:type_name -> dnd5e.api.v1alpha1.Subclass
	125, // 89: dnd5e.api.v1alpha1.UpdateBackgroundRequest.background:type_name -> dnd5e.api.v1alpha1.Background
	141, // 90: dnd5e.api.v1alpha1.UpdateBackgroundRequest.background_choices:type_name -> dnd5e.api.v1alpha1.ChoiceData
	127, // 91: dnd5e.api.v1alpha1.UpdateAbilityScoresRequest.ability_scores:type_name -> dnd5e.api.v1alpha1.AbilityScores
	96,  // 92: dnd5e.api.v1alpha1.UpdateAbilityScoresRequest.roll_assignments:type_name -> dnd5e.api.v1alpha1.RollAssignments
	136, // 93: dnd5e.api.v1alpha1.UpdateSkillsRequest.skills:type_name -> dnd5e.api.v1alpha1.Skill
	34,  // 94: dnd5e.api.v1alpha1.UpdateNameResponse.draft:type_name -> dnd5e.api.v1alpha1.CharacterDraft
	34,  // 95: dnd5e.api.v1alpha1.UpdateRaceResponse.draft:type_name -> dnd5e.api.v1alpha1.CharacterDraft
	34,  // 96: dnd5e.api.v1alpha1.UpdateClassResponse.draft:type_name -> dnd5e.api.v1alpha1.CharacterDraft
	34,  // 97: dnd5e.api.v1alpha1.UpdateBackgroundResponse.draft:type_name -> dnd5e.api.v1alpha1.CharacterDraft
	34,  // 98: dnd5e.api.v1alpha1.UpdateAbilityScoresResponse.draft:type_name -> dnd5e.api.v1alpha1.CharacterDraft
	34,  // 99: dnd5e.api.v1alpha1.UpdateSkillsResponse.draft:type_name -> dnd5e.api.v1alpha1.CharacterDraft
	11,  // 100: dnd5e.api.v1alpha1.UpdateAppearanceRequest.appearance:type_name -> dnd5e.api.v1alpha1.Appearance
	34,  // 101: dnd5e.api.v1alpha1.UpdateAppearanceResponse.draft:type_name -> dnd5e.api.v1alpha1.CharacterDraft
	34,  // 102: dnd5e.api.v1alpha1.ListDraftsResponse.drafts:type_name -> dnd5e.api.v1alpha1.CharacterDraft
	143, // 103: dnd5e.api.v1alpha1.ValidateDraftResponse.validation:type_name -> dnd5e.api.v1alpha1.ValidationResult
	34,  // 104: dnd5e.api.v1alpha1.GetDraftPreviewResponse.draft:type_name -> dnd5e.api.v1alpha1.CharacterDraft
	10,  // 105: dnd5e.api.v1alpha1.GetDraftPreviewResponse.preview:type_name -> dnd5e.api.v1alpha1.Character
	10,  // 106: dnd5e.api.v1alpha1.FinalizeDraftResponse.character:type_name -> dnd5e.api.v1alpha1.Character
	143, // 107: dnd5e.api.v1alpha1.FinalizeDraftResponse.final_validation:type_name -> dnd5e.api.v1alpha1.ValidationResult
	122, // 108: dnd5e.api.v1alpha1.RaceInfo.race_id:type_name -> dnd5e.api.v1alpha1.Race
	4,   // 109: dnd5e.api.v1alpha1.RaceInfo.size:type_name -> dnd5e.api.v1alpha1.Size
	118, // 110: dnd5e.api.v1alpha1.RaceInfo.ability_bonuses:type_name -> dnd5e.api.v1alpha1.RaceInfo.AbilityBonusesEntry
	67,  // 111: dnd5e.api.v1alpha1.RaceInfo.traits:type_name -> dnd5e.api.v1alpha1.RacialTrait
	66,  // 112: dnd5e.api.v1alpha1.RaceInfo.subraces:type_name -> dnd5e.api.v1alpha1.SubraceInfo
	14,  // 113: dnd5e.api.v1alpha1.RaceInfo.proficiency_grants:type_name -> dnd5e.api.v1alpha1.ProficiencyGrants
	128, // 114: dnd5e.api.v1alpha1.RaceInfo.languages:type_name -> dnd5e.api.v1alpha1.Language
	140, // 115: dnd5e.api.v1alpha1.RaceInfo.choices:type_name -> dnd5e.api.v1alpha1.Choice
	123, // 116: dnd5e.api.v1alpha1.SubraceInfo.subrace_id:type_name -> dnd5e.api.v1alpha1.Subrace
	119, // 117: dnd5e.api.v1alpha1.SubraceInfo.ability_bonuses:type_name -> dnd5e.api.v1alpha1.SubraceInfo.AbilityBonusesEntry
	67,  // 118: dnd5e.api.v1alpha1.SubraceInfo.traits:type_name -> dnd5e.api.v1alpha1.RacialTrait
	128, // 119: dnd5e.api.v1alpha1.SubraceInfo.languages:type_name -> dnd5e.api.v1alpha1.Language
	14,  // 120: dnd5e.api.v1alpha1.SubraceInfo.proficiency_grants:type_name -> dnd5e.api.v1alpha1.ProficiencyGrants
	124, // 121: dnd5e.api.v1alpha1.ClassInfo.class_id:type_name -> dnd5e.api.v1alpha1.Class
	137, // 122: dnd5e.api.v1alpha1.ClassInfo.primary_ability:type_name -> dnd5e.api.v1alpha1.Ability
	138, // 123: dnd5e.api.v1alpha1.ClassInfo.armor_proficiency_categories:type_name -> dnd5e.api.v1alpha1.ArmorProficiencyCategory
	139, // 124: dnd5e.api.v1alpha1.ClassInfo.weapon_proficiency_categories:type_name -> dnd5e.api.v1alpha1.WeaponProficiencyCategory
	134, // 125: dnd5e.api.v1alpha1.ClassInfo.specific_weapon_proficiencies:type_name -> dnd5e.api.v1alpha1.Weapon
	135, // 126: dnd5e.api.v1alpha1.ClassInfo.tool_proficiencies:type_name -> dnd5e.api.v1alpha1.Tool
	137, // 127: dnd5e.api.v1alpha1.ClassInfo.saving_throw_proficiencies:type_name -> dnd5e.api.v1alpha1.Ability
	73,  // 128: dnd5e.api.v1alpha1.ClassInfo.level_1_features:type_name -> dnd5e.api.v1alpha1.FeatureInfo
	70,  // 129: dnd5e.api.v1alpha1.ClassInfo.spellcasting:type_name -> dnd5e.api.v1alpha1.SpellcastingInfo
	140, // 130: dnd5e.api.v1alpha1.ClassInfo.choices:type_name -> dnd5e.api.v1alpha1.Choice
	69,  // 131: dnd5e.api.v1alpha1.ClassInfo.subclasses:type_name -> dnd5e.api.v1alpha1.SubclassInfo
	142, // 132: dnd5e.api.v1alpha1.SubclassInfo.subclass_id:type_name -> dnd5e.api.v1alpha1.Subclass
	133, // 133: dnd5e.api.v1alpha1.SubclassInfo.armor_proficiencies:type_name -> dnd5e.api.v1alpha1.Armor
	134, // 134: dnd5e.api.v1alpha1.SubclassInfo.weapon_proficiencies:type_name -> dnd5e.api.v1alpha1.Weapon
	135, // 135: dnd5e.api.v1alpha1.SubclassInfo.tool_proficiencies:type_name -> dnd5e.api.v1alpha1.Tool
	140, // 136: dnd5e.api.v1alpha1.SubclassInfo.additional_choices:type_name -> dnd5e.api.v1alpha1.Choice
	73,  // 137: dnd5e.api.v1alpha1.SubclassInfo.features:type_name -> dnd5e.api.v1alpha1.FeatureInfo
	144, // 138: dnd5e.api.v1alpha1.SpellInfo.spell_id:type_name -> dnd5e.api.v1alpha1.Spell
	125, // 139: dnd5e.api.v1alpha1.BackgroundInfo.background_id:type_name -> dnd5e.api.v1alpha1.Background
	136, // 140: dnd5e.api.v1alpha1.BackgroundInfo.skill_proficiencies:type_name -> dnd5e.api.v1alpha1.Skill
	135, // 141: dnd5e.api.v1alpha1.BackgroundInfo.tool_proficiencies:type_name -> dnd5e.api.v1alpha1.Tool
	128, // 142: dnd5e.api.v1alpha1.BackgroundInfo.languages:type_name -> dnd5e.api.v1alpha1.Language
	145, // 143: dnd5e.api.v1alpha1.BackgroundInfo.starting_equipment:type_name -> dnd5e.api.v1alpha1.EquipmentSelection
	140, // 144: dnd5e.api.v1alpha1.BackgroundInfo.choices:type_name -> dnd5e.api.v1alpha1.Choice
	140, // 145: dnd5e.api.v1alpha1.FeatureInfo.choices:type_name -> dnd5e.api.v1alpha1.Choice
	74,  // 146: dnd5e.api.v1alpha1.FeatureInfo.spell_selection:type_name -> dnd5e.api.v1alpha1.SpellSelectionInfo
	146, // 147: dnd5e.api.v1alpha1.SpellSelectionInfo.selection_type:type_name -> dnd5e.api.v1alpha1.SpellSelectionType
	65,  // 148: dnd5e.api.v1alpha1.ListRacesResponse.races:type_name -> dnd5e.api.v1alpha1.RaceInfo
	68,  // 149: dnd5e.api.v1alpha1.ListClassesResponse.classes:type_name -> dnd5e.api.v1alpha1.ClassInfo
	72,  // 150: dnd5e.api.v1alpha1.ListBackgroundsResponse.backgrounds:type_name -> dnd5e.api.v1alpha1.BackgroundInfo
	65,  // 151: dnd5e.api.v1alpha1.GetRaceDetailsResponse.race:type_name -> dnd5e.api.v1alpha1.RaceInfo
	68,  // 152: dnd5e.api.v1alpha1.GetClassDetailsResponse.class:type_name -> dnd5e.api.v1alpha1.ClassInfo
	72,  // 153: dnd5e.api.v1alpha1.GetBackgroundDetailsResponse.background:type_name -> dnd5e.api.v1alpha1.BackgroundInfo
	73,  // 154: dnd5e.api.v1alpha1.GetFeatureResponse.feature:type_name -> dnd5e.api.v1alpha1.FeatureInfo
	90,  // 155: dnd5e.api.v1alpha1.RollAbilityScoresResponse.rolls:type_name -> dnd5e.api.v1alpha1.AbilityScoreRoll
	124, // 156: dnd5e.api.v1alpha1.GetRequirementsRequest.class:type_name -> dnd5e.api.v1alpha1.Class
	122, // 157: dnd5e.api.v1alpha1.GetRequirementsRequest.race:type_name -> dnd5e.api.v1alpha1.Race
	140, // 158: dnd5e.api.v1alpha1.GetRequirementsResponse.requirements:type_name -> dnd5e.api.v1alpha1.Choice
	147, // 159: dnd5e.api.v1alpha1.SubmitChoicesRequest.submissions:type_name -> dnd5e.api.v1alpha1.ChoiceSubmission
	34,  // 160: dnd5e.api.v1alpha1.SubmitChoicesResponse.draft:type_name -> dnd5e.api.v1alpha1.CharacterDraft
	143, // 161: dnd5e.api.v1alpha1.SubmitChoicesResponse.validation:type_name -> dnd5e.api.v1alpha1.ValidationResult
	140, // 162: dnd5e.api.v1alpha1.SubmitChoicesResponse.remaining_requirements:type_name -> dnd5e.api.v1alpha1.Choice
	98,  // 163: dnd5e.api.v1alpha1.EquipmentSlots.main_hand:type_name -> dnd5e.api.v1alpha1.InventoryItem
	98,  // 164: dnd5e.api.v1alpha1.EquipmentSlots.off_hand:type_name -> dnd5e.api.v1alpha1.InventoryItem
	98,  // 165: dnd5e.api.v1alpha1.EquipmentSlots.armor:type_name -> dnd5e.api.v1alpha1.InventoryItem
	98,  // 166: dnd5e.api.v1alpha1.EquipmentSlots.helmet:type_name -> dnd5e.api.v1alpha1.InventoryItem
	98,  // 167: dnd5e.api.v1alpha1.EquipmentSlots.boots:type_name -> dnd5e.api.v1alpha1.InventoryItem
	98,  // 168: dnd5e.api.v1alpha1.EquipmentSlots.gloves:type_name -> dnd5e.api.v1alpha1.InventoryItem
	98,  // 169: dnd5e.api.v1alpha1.EquipmentSlots.cloak:type_name -> dnd5e.api.v1alpha1.InventoryItem
	98,  // 170: dnd5e.api.v1alpha1.EquipmentSlots.amulet:type_name -> dnd5e.api.v1alpha1.InventoryItem
	98,  // 171: dnd5e.api.v1alpha1.EquipmentSlots.ring_1:type_name -> dnd5e.api.v1alpha1.InventoryItem
	98,  // 172: dnd5e.api.v1alpha1.EquipmentSlots.ring_2:type_name -> dnd5e.api.v1alpha1.InventoryItem
	98,  // 173: dnd5e.api.v1alpha1.EquipmentSlots.belt:type_name -> dnd5e.api.v1alpha1.InventoryItem
	148, // 174: dnd5e.api.v1alpha1.InventoryItem.equipment:type_name -> dnd5e.api.v1alpha1.Equipment
	5,   // 175: dnd5e.api.v1alpha1.EncumbranceInfo.level:type_name -> dnd5e.api.v1alpha1.EncumbranceLevel
	149, // 176: dnd5e.api.v1alpha1.SpellDamage.damage_type:type_name -> dnd5e.api.v1alpha1.DamageType
	101, // 177: dnd5e.api.v1alpha1.SpellDamage.damage_at_slot_level:type_name -> dnd5e.api.v1alpha1.DamageAtSlotLevel
	150, // 178: dnd5e.api.v1alpha1.ListEquipmentByTypeRequest.equipment_type:type_name -> dnd5e.api.v1alpha1.EquipmentType
	148, // 179: dnd5e.api.v1alpha1.ListEquipmentByTypeResponse.equipment:type_name -> dnd5e.api.v1alpha1.Equipment
	124, // 180: dnd5e.api.v1alpha1.ListSpellsByLevelRequest.class:type_name -> dnd5e.api.v1alpha1.Class
	71,  // 181: dnd5e.api.v1alpha1.ListSpellsByLevelResponse.spells:type_name -> dnd5e.api.v1alpha1.SpellInfo
	97,  // 182: dnd5e.api.v1alpha1.GetCharacterInventoryResponse.equipment_slots:type_name -> dnd5e.api.v1alpha1.EquipmentSlots
	98,  // 183: dnd5e.api.v1alpha1.GetCharacterInventoryResponse.inventory:type_name -> dnd5e.api.v1alpha1.InventoryItem
	99,  // 184: dnd5e.api.v1alpha1.GetCharacterInventoryResponse.encumbrance:type_name -> dnd5e.api.v1alpha1.EncumbranceInfo
	6,   // 185: dnd5e.api.v1alpha1.EquipItemRequest.slot:type_name -> dnd5e.api.v1alpha1.EquipmentSlot
	10,  // 186: dnd5e.api.v1alpha1.EquipItemResponse.character:type_name -> dnd5e.api.v1alpha1.Character
	98,  // 187: dnd5e.api.v1alpha1.EquipItemResponse.previously_equipped_item:type_name -> dnd5e.api.v1alpha1.InventoryItem
	6,   // 188: dnd5e.api.v1alpha1.UnequipItemRequest.slot:type_name -> dnd5e.api.v1alpha1.EquipmentSlot
	10,  // 189: dnd5e.api.v1alpha1.UnequipItemResponse.character:type_name -> dnd5e.api.v1alpha1.Character
	114, // 190: dnd5e.api.v1alpha1.AddToInventoryRequest.items:type_name -> dnd5e.api.v1alpha1.InventoryAddition
	10,  // 191: dnd5e.api.v1alpha1.AddToInventoryResponse.character:type_name -> dnd5e.api.v1alpha1.Character
	10,  // 192: dnd5e.api.v1alpha1.RemoveFromInventoryResponse.character:type_name -> dnd5e.api.v1alpha1.Character
	35,  // 193: dnd5e.api.v1alpha1.CharacterService.CreateDraft:input_type -> dnd5e.api.v1alpha1.CreateDraftRequest
	37,  // 194: dnd5e.api.v1alpha1.CharacterService.GetDraft:input_type -> dnd5e.api.v1alpha1.GetDraftRequest
	55,  // 195: dnd5e.api.v1alpha1.CharacterService.ListDrafts:input_type -> dnd5e.api.v1alpha1.ListDraftsRequest
	63,  // 196: dnd5e.api.v1alpha1.CharacterService.DeleteDraft:input_type -> dnd5e.api.v1alpha1.DeleteDraftRequest
	41,  // 197: dnd5e.api.v1alpha1.CharacterService.UpdateName:input_type -> dnd5e.api.v1alpha1.UpdateNameRequest
	42,  // 198: dnd5e.api.v1alpha1.CharacterService.UpdateRace:input_type -> dnd5e.api.v1alpha1.UpdateRaceRequest
	43,  // 199: dnd5e.api.v1alpha1.CharacterService.UpdateClass:input_type -> dnd5e.api.v1alpha1.UpdateClassRequest
	44,  // 200: dnd5e.api.v1alpha1.CharacterService.UpdateBackground:input_type -> dnd5e.api.v1alpha1.UpdateBackgroundRequest
	45,  // 201: dnd5e.api.v1alpha1.CharacterService.UpdateAbilityScores:input_type -> dnd5e.api.v1alpha1.UpdateAbilityScoresRequest
	46,  // 202: dnd5e.api.v1alpha1.CharacterService.UpdateSkills:input_type -> dnd5e.api.v1alpha1.UpdateSkillsRequest
	53,  // 203: dnd5e.api.v1alpha1.CharacterService.UpdateAppearance:input_type -> dnd5e.api.v1alpha1.UpdateAppearanceRequest
	57,  // 204: dnd5e.api.v1alpha1.CharacterService.ValidateDraft:input_type -> dnd5e.api.v1alpha1.ValidateDraftRequest
	59,  // 205: dnd5e.api.v1alpha1.CharacterService.GetDraftPreview:input_type -> dnd5e.api.v1alpha1.GetDraftPreviewRequest
	61,  // 206: dnd5e.api.v1alpha1.CharacterService.FinalizeDraft:input_type -> dnd5e.api.v1alpha1.FinalizeDraftRequest
	17,  // 207: dnd5e.api.v1alpha1.CharacterService.GetCharacter:input_type -> dnd5e.api.v1alpha1.GetCharacterRequest
	19,  // 208: dnd5e.api.v1alpha1.CharacterService.ListCharacters:input_type -> dnd5e.api.v1alpha1.ListCharactersRequest
	23,  // 209: dnd5e.api.v1alpha1.CharacterService.DeleteCharacter:input_type -> dnd5e.api.v1alpha1.DeleteCharacterRequest
	25,  // 210: dnd5e.api.v1alpha1.CharacterService.GetNextLevel:input_type -> dnd5e.api.v1alpha1.GetNextLevelRequest
	27,  // 211: dnd5e.api.v1alpha1.CharacterService.LevelUp:input_type -> dnd5e.api.v1alpha1.LevelUpRequest
	75,  // 212: dnd5e.api.v1alpha1.CharacterService.ListRaces:input_type -> dnd5e.api.v1alpha1.ListRacesRequest
	77,  // 213: dnd5e.api.v1alpha1.CharacterService.ListClasses:input_type -> dnd5e.api.v1alpha1.ListClassesRequest
	79,  // 214: dnd5e.api.v1alpha1.CharacterService.ListBackgrounds:input_type -> dnd5e.api.v1alpha1.ListBackgroundsRequest
	81,  // 215: dnd5e.api.v1alpha1.CharacterService.GetRaceDetails:input_type -> dnd5e.api.v1alpha1.GetRaceDetailsRequest
	83,  // 216: dnd5e.api.v1alpha1.CharacterService.GetClassDetails:input_type -> dnd5e.api.v1alpha1.GetClassDetailsRequest
	85,  // 217: dnd5e.api.v1alpha1.CharacterService.GetBackgroundDetails:input_type -> dnd5e.api.v1alpha1.GetBackgroundDetailsRequest
	87,  // 218: dnd5e.api.v1alpha1.CharacterService.GetFeature:input_type -> dnd5e.api.v1alpha1.GetFeatureRequest
	89,  // 219: dnd5e.api.v1alpha1.CharacterService.RollAbilityScores:input_type -> dnd5e.api.v1alpha1.RollAbilityScoresRequest
	92,  // 220: dnd5e.api.v1alpha1.CharacterService.GetRequirements:input_type -> dnd5e.api.v1alpha1.GetRequirementsRequest
	94,  // 221: dnd5e.api.v1alpha1.CharacterService.SubmitChoices:input_type -> dnd5e.api.v1alpha1.SubmitChoicesRequest
	103, // 222: dnd5e.api.v1alpha1.CharacterService.ListEquipmentByType:input_type -> dnd5e.api.v1alpha1.ListEquipmentByTypeRequest
	105, // 223: dnd5e.api.v1alpha1.CharacterService.ListSpellsByLevel:input_type -> dnd5e.api.v1alpha1.ListSpellsByLevelRequest
	107, // 224: dnd5e.api.v1alpha1.CharacterService.GetCharacterInventory:input_type -> dnd5e.api.v1alpha1.GetCharacterInventoryRequest
	109, // 225: dnd5e.api.v1alpha1.CharacterService.EquipItem:input_type -> dnd5e.api.v1alpha1.EquipItemRequest
	111, // 226: dnd5e.api.v1alpha1.CharacterService.UnequipItem:input_type -> dnd5e.api.v1alpha1.UnequipItemRequest
	113, // 227: dnd5e.api.v1alpha1.CharacterService.AddToInventory:input_type -> dnd5e.api.v1alpha1.AddToInventoryRequest
	116, // 228: dnd5e.api.v1alpha1.CharacterService.RemoveFromInventory:input_type -> dnd5e.api.v1alpha1.RemoveFromInventoryRequest
	36,  // 229: dnd5e.api.v1alpha1.CharacterService.CreateDraft:output_type -> dnd5e.api.v1alpha1.CreateDraftResponse
	38,  // 230: dnd5e.api.v1alpha1.CharacterService.GetDraft:output_type -> dnd5e.api.v1alpha1.GetDraftResponse
	56,  // 231: dnd5e.api.v1alpha1.CharacterService.ListDrafts:output_type -> dnd5e.api.v1alpha1.ListDraftsResponse
	64,  // 232: dnd5e.api.v1alpha1.CharacterService.DeleteDraft:output_type -> dnd5e.api.v1alpha1.DeleteDraftResponse
	47,  // 233: dnd5e.api.v1alpha1.CharacterService.UpdateName:output_type -> dnd5e.api.v1alpha1.UpdateNameResponse
	48,  // 234: dnd5e.api.v1alpha1.CharacterService.UpdateRace:output_type -> dnd5e.api.v1alpha1.UpdateRaceResponse
	49,  // 235: dnd5e.api.v1alpha1.CharacterService.UpdateClass:output_type -> dnd5e.api.v1alpha1.UpdateClassResponse
	50,  // 236: dnd5e.api.v1alpha1.CharacterService.UpdateBackground:output_type -> dnd5e.api.v1alpha1.UpdateBackgroundResponse
	51,  // 237: dnd5e.api.v1alpha1.CharacterService.UpdateAbilityScores:output_type -> dnd5e.api.v1alpha1.UpdateAbilityScoresResponse
	52,  // 238: dnd5e.api.v1alpha1.CharacterService.UpdateSkills:output_type -> dnd5e.api.v1alpha1.UpdateSkillsResponse
	54,  // 239: dnd5e.api.v1alpha1.CharacterService.UpdateAppearance:output_type -> dnd5e.api.v1alpha1.UpdateAppearanceResponse
	58,  // 240: dnd5e.api.v1alpha1.CharacterService.ValidateDraft:output_type -> dnd5e.api.v1alpha1.ValidateDraftResponse
	60,  // 241: dnd5e.api.v1alpha1.CharacterService.GetDraftPreview:output_type -> dnd5e.api.v1alpha1.GetDraftPreviewResponse
	62,  // 242: dnd5e.api.v1alpha1.CharacterService.FinalizeDraft:output_type -> dnd5e.api.v1alpha1.FinalizeDraftResponse
	18,  // 243: dnd5e.api.v1alpha1.CharacterService.GetCharacter:output_type -> dnd5e.api.v1alpha1.GetCharacterResponse
	20,  // 244: dnd5e.api.v1alpha1.CharacterService.ListCharacters:output_type -> dnd5e.api.v1alpha1.ListCharactersResponse
	24,  // 245: dnd5e.api.v1alpha1.CharacterService.DeleteCharacter:output_type -> dnd5e.api.v1alpha1.DeleteCharacterResponse
	26,  // 246: dnd5e.api.v1alpha1.CharacterService.GetNextLevel:output_type -> dnd5e.api.v1alpha1.GetNextLevelResponse
	28,  // 247: dnd5e.api.v1alpha1.CharacterService.LevelUp:output_type -> dnd5e.api.v1alpha1.LevelUpResponse
	76,  // 248: dnd5e.api.v1alpha1.CharacterService.ListRaces:output_type -> dnd5e.api.v1alpha1.ListRacesResponse
	78,  // 249: dnd5e.api.v1alpha1.CharacterService.ListClasses:output_type -> dnd5e.api.v1alpha1.ListClassesResponse
	80,  // 250: dnd5e.api.v1alpha1.CharacterService.ListBackgrounds:output_type -> dnd5e.api.v1alpha1.ListBackgroundsResponse
	82,  // 251: dnd5e.api.v1alpha1.CharacterService.GetRaceDetails:output_type -> dnd5e.api.v1alpha1.GetRaceDetailsResponse
	84,  // 252: dnd5e.api.v1alpha1.CharacterService.GetClassDetails:output_type -> dnd5e.api.v1alpha1.GetClassDetailsResponse
	86,  // 253: dnd5e.api.v1alpha1.CharacterService.GetBackgroundDetails:output_type -> dnd5e.api.v1alpha1.GetBackgroundDetailsResponse
	88,  // 254: dnd5e.api.v1alpha1.CharacterService.GetFeature:output_type -> dnd5e.api.v1alpha1.GetFeatureResponse
	91,  // 255: dnd5e.api.v1alpha1.CharacterService.RollAbilityScores:output_type -> dnd5e.api.v1alpha1.RollAbilityScoresResponse
	93,  // 256: dnd5e.api.v1alpha1.CharacterService.GetRequirements:output_type -> dnd5e.api.v1alpha1.GetRequirementsResponse
	95,  // 257: dnd5e.api.v1alpha1.CharacterService.SubmitChoices:output_type -> dnd5e.api.v1alpha1.SubmitChoicesResponse
	104, // 258: dnd5e.api.v1alpha1.CharacterService.ListEquipmentByType:output_type -> dnd5e.api.v1alpha1.ListEquipmentByTypeResponse
	106, // 259: dnd5e.api.v1alpha1.CharacterService.ListSpellsByLevel:output_type -> dnd5e.api.v1alpha1.ListSpellsByLevelResponse
	108, // 260: dnd5e.api.v1alpha1.CharacterService.GetCharacterInventory:output_type -> dnd5e.api.v1alpha1.GetCharacterInventoryResponse
	110, // 261: dnd5e.api.v1alpha1.CharacterService.EquipItem:output_type -> dnd5e.api.v1alpha1.EquipItemResponse
	112, // 262: dnd5e.api.v1alpha1.CharacterService.UnequipItem:output_type -> dnd5e.api.v1alpha1.UnequipItemResponse
	115, // 263: dnd5e.api.v1alpha1.CharacterService.AddToInventory:output_type -> dnd5e.api.v1alpha1.AddToInventoryResponse
	117, // 264: dnd5e.api.v1alpha1.CharacterService.RemoveFromInventory:output_type -> dnd5e.api.v1alpha1.RemoveFromInventoryResponse
	229, // [229:265] is the sub-list for method output_type
	193, // [193:229] is the sub-list for method input_type
...
func file_dnd5e_api_v1alpha1_character_proto_init() {
	...
	file_dnd5e_api_v1alpha1_choices_proto_init()
	file_dnd5e_api_v1alpha1_common_proto_init()
	file_dnd5e_api_v1alpha1_enums_proto_init()
	file_dnd5e_api_v1alpha1_equipment_types_proto_init()
	file_dnd5e_api_v1alpha1_character_proto_msgTypes[38].OneofWrappers = []any{
		(*UpdateAbilityScoresRequest_AbilityScores)(nil),
		(*UpdateAbilityScoresRequest_RollAssignments)(nil),
	}
	file_dnd5e_api_v1alpha1_character_proto_msgTypes[109].OneofWrappers = []any{
